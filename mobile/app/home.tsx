...
import { WarmCard } from '../src/components/WarmCard'
...
import { PauseIcon, ChatIcon, MapPinIcon, BellIcon, WifiOffIcon, SignOutIcon, BlockIcon, InboxIcon, GlyphScale, CloseIcon, UserIcon, CameraIcon, ShieldIcon, GroupsIcon, SearchIcon, CreditIcon } from '../src/components/icons'
import type { CardAction, CardCountdown } from '../src/components/MatchCard'
import { AppStatusBar } from '../src/components/AppStatusBar'


// â”€â”€ Avatar rings: static halo + radar pulse â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€

// The one action surface on home. Everything around it â€” the halo, the dotted
// ring, the glyph inside â€” derives from this number, so the whole centre grows
...
function RadarRing({ active, ringIndex }: { active: boolean; ringIndex: number }) {
  ...
  const style = useAnimatedStyle(() => {
    ...
    // Ring is brightest right at the photo edge (tâ‰ˆ0) and dissolves
    // smoothly with a cubic ease-out as it sweeps outward â€” classic ripple.
    const out = 1 - t
...
// deep-wine INK page â€” INK-on-INK was invisible.)
...
// its own making â€” the headline centred in everything above the circle, the
// circle on the middle line, the art centred in everything below it â€” and each
...
// The longest an answered invitation may be held on the screen waiting for the
// conversation to cover it (see `answeredInvite`). Not choreography â€” every
// ordinary route out of that hold is an event, and this is only the floor under
// the routes nobody can name: a Realtime event that never lands, an app
// backgrounded through the whole transition. Long on purpose: the accept's own
// round trip, three server events and a sheet's rise measured ~2.5s on an
// emulator, and firing early would put back the very flash the hold exists to
// prevent.
const ANSWER_HOLD_MAX_MS = 15_000
...
// The headline's letter tracking â€” this screen's heading type.
const SKIP_HINT_TRACKING = 2
// The headline's symmetric side gutter (user directive 2026-07-30). The SVG is
// sized to (window âˆ’ 2 Ã— HPAD), so this is the one number that decides where a
...
//
// It was MD * 2 while the label shrank to fit â€” wide side margins cost nothing
// when the font is free to shrink â€” then MD once the size was fixed, since every
...
// line â€” the box hugs the lines it actually drew (see HeadlineArea).
...
//   READY_HEADLINES â€” shown in the headline slot above the centre button
//     while the home pane is in the ready state (no card).
//   SKIP_HEADLINES  â€” skip-feedback lines; one is parked in that same slot
...
// punctuation (it is dropped, not moved to the end of the first line) â€” and
...
function splitAtPause(text: string): string[] | null {
  ...
  for (let i = 0; i < text.length - 1; i++) {
    // Followed by a space â‡’ a real pause, not a decimal point or an
...
// headline â€” the same slot visibly changing size with its content. Now the
...
// band above it (permHeadlineBand) â€” a reserved height would simply centre the
...
// BELOW the circle now â€” see `permArtBand` â€” and the two things that used to
...
// <SheetActions flush> like every other popup's â€” see the sheet itself below.)

// â”€â”€ Invite timer â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
// The visible countdown is NOT here any more (user directive 2026-07-30): it
// rides inside the name/age chip on the card itself â€” see CardCountdown in
...
// second â€” it schedules a single timeout for the exact remaining interval and
...
// paint â€” no flash, no wait for the hourly cron teardown or a network fetch.
...
// â”€â”€ StatusCard scaffold â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// a stack of full-width Buttons below â€” and NOTHING between the two (user
...
const statusCardStyles = StyleSheet.create({
  ...
  // its top air and its bottom gap are the sheet's â€” one set of values for every
  ...
  // BottomSheet.tsx) â€” the same size, the same full-strength ink, the same
  ...
  // 2026-07-26), and it is THE popup title (SHEET_TITLE) â€” a status card's
...
// body paragraph below it (user directive 2026-07-26 â€” heading was previously
...
// (CLAUDE.md "i18n text style") â€” `?` and `!` are left alone, they are the
// heading's own voice ("×œ×”×–×ž×™×Ÿ ××•×ª×” ×œ×¦×³××˜?").
...
// and their paragraphs are gone â€” for every one of those states the paragraph
// restated the title and then said in words what the chip now says as a verb
// ("××¤×©×¨ ×œ×”×ž×©×™×š ×”×œ××”" â†’ "×”×ž×©×š"). What is left of the scaffold below â€”
// `StatusCardText`, `statusCardStyles`, `statusButtonStyles` â€” belongs to
...
// â”€â”€ The card's message, as a popup â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// and the ONE thing to do about it. It is a popup body now â€” raised by the
// chevron beside the clock on the card's heading chip â€” because the message was
...
// â”€â”€ ReplyingInviteCard â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// (gem Ã— N) so what the press spends is legible at the moment of pressing.
//
// It used to be BOTH sides of an invitation â€” the incoming one was this same
// card standing at the top of the page2 profile, with a ghost decline beside the
// accept â€” and that side is now two chips in the card's own strip, so the
...
function ReplyingInviteCard({
   ...
   * 2026-07-25: no Skip button there â€” the card is dismissed by swiping the
   * sheet down, and a person is skipped by the page1 pull gesture instead). */
  declineLabel?: string
  /** Stars the accept action spends â€” shown as the in-button cost badge
   * (heart + N). Reads "0" on the free page1 invite prompt so the user
   * sees that inviting costs nothing. */
  costCredits: number
  /** False â‡’ the user can't afford the accept. When `onUnaffordable` is
   ...
   * 2026-08-02) â€” nothing behind this button asks the same question again. */
  onDecline?: () => void
   ...
   * the plain disabled fade â€” the app's other wait, the one with nothing in it. */
   ...
   * it â€” the tinted form was the incoming-invitation card, which sat straight on
        ...
        <View style={statusButtonStyles.btnRow}>
          {declineLabel != null && onDecline != null && (
            <View style={statusButtonStyles.btnDecline}>
              <Button
                ...
                // `loading` here â€” see `declineLoading`.
          ...
          <View style={statusButtonStyles.btnAccept}>
            {/* The invite keeps tone="positive" for intent, but since the app
                ...
                purple as every other button â€” no separate action hue anymore. */}
            <Button
              variant="primary"
              tone="positive"
              label={acceptLabel}
...
const statusButtonStyles = StyleSheet.create({
  ...
  // animates its stack (`layout={STATUS_LAYOUT}`) â€” the VALUE is the shared one.
  stack: {
...
export default function HomePage() {
  ...
  // â€¦AND HE MAY WATCH TWO PEOPLE BEFORE THAT (user directive 2026-07-31). The
  ...
  // candidate could arrive by â€” the skip's commit and the promote effect.
  const browse = useBrowseAllowance(profileBuilt)
  // Read from the pull's commit, which must not change identity because a face
  // was counted â€” assigned during render, like overlaysRef.
  const browseSpentRef = useRef(false)
  browseSpentRef.current = browse.spent
  // â”€â”€ Single-screen shell â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  // The app is ONE screen: page1 (home). Everything else rises over it as a
  // full-screen OverlaySheet and is dismissed by swiping down â€” the same
  // gesture, from the same machinery, as page1's pull-to-skip.
  //
  // Two kinds of overlay, deliberately modelled differently:
  //
  //   STACKED (this state) â€” user-driven surfaces the user opens and closes:
  ...
  //
  //   DERIVED (not in this state) â€” the incoming-invite / dead-invite card.
  ...
  //
  // Paint order is low â†’ high: home < invite < chat < profile sheet.
  //
  ...
  //   chat codes  â†’ the chat overlay
  //   page2 codes â†’ nothing to route: the invite overlay is DERIVED, so it is
  //                 already up the moment the store hydrates
  //   page1 codes â†’ home, i.e. no overlay
  ...
  // Friend-lifecycle pushes open the friends list â€” where an incoming request's
  ...
  // First-render only: routing for the notification that LAUNCHED the app â€” a
  ...
  // button is the way in. stateâ†’chat transitions during the session are owned
  ...
  // Unread message count reported by ChatPage â€” drives the alert pulse on the
  ...
  // this works while the chat is CLOSED â€” exactly when the dot is needed. See
  // useChatHasUnread.
  ...
  // relations.communities summary the settings row already counts from â€” one
  ...
  // QUANTITY, and the dock says a quantity with a chip rather than a dot â€” see
  // the dock block below.
  const pendingCount = pendingApprovals(circlesSummary(profile))
  // â”€â”€ "You haven't met your circles yet" â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  // deleted (user directive 2026-07-30) â€” nothing takes the screen away from a
  ...
  // my approval) can light it again â€” which is the badge's other half below.
  //
  ...
  // initial notification) and one of them is refused outright â€” a user with no
  ...
  // saw the page â€” and, being persisted, it burned it forever: he finished his
  ...
  // Whether the chat partner is currently online â€” reported by ChatPage via
  ...
  // is correctly sized on its first render â€” the card then rises as one solid
  ...
  // Cleared on the openâ†’closed TRANSITION only. Not "whenever it is closed":
  ...
  // backgrounded through the whole transition. Generous on purpose: it must be
  // longer than the slowest legitimate answer (the round trip, the event, the
  // sheet's mount and its rise), because firing early is the very flash this
  // exists to prevent.
  useEffect(() => {
    if (!answeredInvite) return
    const t = setTimeout(() => setAnsweredInvite(null), ANSWER_HOLD_MAX_MS)
  ...
  // â”€â”€ Geo-availability gate â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  // Absent / 'available' â†’ normal behaviour (also the no-areas-configured
  // case, so this is fully backward compatible).
  //
  // Scoped to the idle world: an active chat is never gated (a user who
  // matched before being geo-gated keeps their conversation â€” tearing it
  ...
  // for brief phrases â€” a long date string overflows/clips there.
  const gateWhenStr = (() => {
  ...
  // â”€â”€ Notification tap handler â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  // ROUTED entries too â€” a new user who installed through a friend's invite link
  ...
  const routeToCircles = useCallback(() => {
    ...
    // branch â€” without it, the one key in the strip that is dimmed would also be
  ...
  // â”€â”€ Invite deep links â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  // orders â€” a cold start redeems before this mount (the outcome is held for the
  ...
  // just gave â€” a member, or a request waiting on an answer. The hub alone was
  ...
  // nothing to look up; an outcome without one â€” an older server â€” still lands
  ...
  // â”€â”€ The dock: home's four entries â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  // Everything the menu page held, stated on the page itself (user directive
  // 2026-07-30) â€” profile, circles, preferences, more, in that order. Two of them
  ...
  // of the four.
  //
  // **Circles carries the queue** â€” an open request in Circles is said on the
  ...
  // `circlesSeen`) â€” the whole of what the forced onboarding nudge used to do.
  ...
  //   1. no profile yet â†’ the key is FADED (`dimmed`), glyph and word together,
  ...
  //      (`routeToCircles` â†’ `commGateOpen`), which explains the closed door
  //      and offers onboarding.
  //   2. profile built, never opened Circles â†’ full strength, WITH the dot. Not
  //      counted, not delayed: it is there from the moment the profile exists.
  //   3. opened once â†’ clean, until a real queue lights it again (`pendingCount`,
  //      as the purple chip).
  //
  ...
  // picture of a decision made there â€” never a second copy of it.
  //
  // **A QUANTITY is a number; a dot is only for what cannot be counted** (user
  // directive 2026-07-30). Three of the four entries open on a quantity â€” the
  // requests waiting in Circles, the people watching me, the credits I hold â€”
  ...
  // 2026-07-30, after all three wore the app's `Chip` for an afternoon â€” the
  ...
  // an amount â€” zero included, which is the number that matters most.
  ...
  //   1. the topmost stacked overlay (profile sheet â†’ Circles â†’ chat) pops
  ...
  //   3. nothing left â†’ false, i.e. leave the app
  // A step used to stand between 1 and 2 â€” the card in front of the user took
  ...
  // (null state + null match below), so it never renders â€” even on the first
  ...
  // button. The lapse does ask the server to settle (handleInviteLapsed â†’
  // /app/focus â†’ app_expire_self), but the client cannot LEARN the answer from
  ...
  // arrives over the socket alone â€” and when that event is late, dropped, or
  ...
  //
  // What he saw instead, measured on the emulator: accepting Dana's invitation
  // put FOUR different screens in front of him over two and a half seconds â€”
  // because the accept is not one event. The server writes the board through the
  // `watch` rows, and each of those writes is projected onto `users.relations`
  // and broadcast on its own, so the client is handed the transition in stages:
  //
  //   +0.6s  page1 empties      the card he was watching slides out
  //   +1.2s  page2 empties      the invitation goes â€” HOME IS NOW BARE, with the
  //                             play button and "someone worth meeting" on it
  //   +1.5s  page1 = chat       the partner's card rises
  //   +2.5s  the chat sheet finally stands over all of it
  //
  // None of those middle screens is a state the user was ever in. So the card he
  // ANSWERED is what stays on the screen for the whole of it: from the tap until
  // the conversation is standing over it (`answeredInvite`, cleared by the chat
  // sheet's own arrival). Everything page2 draws reads THIS value â€” the clock,
  // the round action, the chip's mark, the sheet itself â€” so the held card is
  // the card he was looking at in every respect, not a copy of it that has to be
  // kept in step. Home's page1 swap and the invite sheet's ride-off both happen
  // behind it, unseen, and by the time the chat can be swiped away home is
  // already the partner's card.
  //
  ...
  // above (`invitePaintedId`), which the sheet's own entrance now waits on.)
  ...
  // â”€â”€ Notification permission flow â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  // Default to 'undetermined' so the in-app prompt is visible from first
  // paint â€” never gate on a `null` "still-checking" state, since any failure
  ...
  // â”€â”€ Permission request handler â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  // Same 'undetermined' default as notifPerm â€” see the comment above.
    ...
    try {
      if (notifPerm !== 'granted') {
        ...
        if (result === 'granted') {
          ...
          // React render batch â€” avoids a window where notifPerm='granted' and
  ...
  // is a deliberate manual point) â†’ null the server location so the user
  // truly leaves everyone's candidate pool (others() excludes null-location).
  // Debounced to the lostâ†’regained transition so it fires once, not per tick.
  ...
  // â”€â”€ Startup completion â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  // Both permissions granted â†’ send app/start + try to get location.
  ...
  // Same idea for app/focus (sent on backgroundâ†’active transitions). The
  ...
  useEffect(() => {
    if (notifPerm !== 'granted') return
    // Custom-location users bypass GPS permission entirely â€” their location
  ...
  // used to happen), and the volley above is fired ONCE per mount â€” so without
  ...
  // â”€â”€ Internet reachability â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  // Unified card mode â€” derived synchronously. The home pane is laid out
  ...
  // showHiddenPlaceholder flips true â†’ the empty pane (with the notice)
  ...
  // availability only â€” no auto-find / snapshot work). Change-debounced
  ...
  // â”€â”€ Re-check permissions when app returns to foreground â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  // Covers the user changing app permissions in device settings, etc.
  // Fires on every backgroundâ†’active transition.
  useEffect(() => {
    ...
    const sub = AppState.addEventListener('change', async (next) => {
      ...
      setNotifPerm(np)
      // Returning from OS Settings is THE moment permission changes â€” report
      ...
      // Skip when the user opted into custom-location mode â€” we don't read GPS
      // perm in that flow.
      ...
      // Custom-location users don't attach a location â€” the server already has
  ...
  // â”€â”€ Poll location services while notif phase is done â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  // hasServicesEnabledAsync is a fast local call â€” negligible battery cost.
  ...
  useEffect(() => {
    ...
    // every 2s forever â€” background work that visibly contended with the
  ...
  // â”€â”€ Poll notification permission while foreground â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  // â‡’ steady state is zero network and zero re-render (state bail-out).
  useEffect(() => {
  ...
  // â”€â”€ Continuous location tracking â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  // a beat later â€” with `prev !== 'chat'` alone, an ordinary relaunch into an
  ...
  // launch-notification effect above). `serverSynced` flips falseâ†’true in the
  ...
  useEffect(() => {
    ...
    if (prevStateRef.current !== state) {
      ...
      if (enteringChat) {
        ...
        // on the device. The server keeps the rows â€” a pair that matches again
        // just re-fetches.
  ...
  // only that one shows the disabled visual â€” all other buttons stay
  ...
  // and mounts the new one â€” Reanimated's SlideOutDown / SlideInDown layout
  ...
  // can't depend on the state directly â€” reading the ref keeps the callback
  ...
  // mid-skip â€” it makes every skip-promote path (the remoteâ†’displayed sync
  ...
  // True only while app/pause (from the skip pause button) is in flight â€”
  ...
  // itself does NOT set this â€” only the post-release server call does, so
  ...
  // copies â€” "Scanning..." while the round-trip is in flight, "Loading
  // profile data" while the (already-resolved) candidate's images load â€”
  ...
  // Search watchdog. `searching` is normally cleared by the remoteâ†’displayed
  // sync effect (or onPreloadReady) once Realtime confirms the find result.
  // If that confirmation never arrives â€” a dropped Realtime relations event,
  // or a server path that returns without changing state â€” the radar/locating
  ...
  // shared value `skipSlide` âˆˆ [0,2] fed to the tab as `nameSlide`:
  //   0 â†’ 1  the candidate's name slides DOWN out while "×œ× ×¢×›×©×™×•" enters
  //          from the TOP â€” tracked 1:1 with the pull, complete at the commit
  //          point (clamp(pullY / commitDistance, 0, 1)).
  //   1      held at "×œ× ×¢×›×©×™×•" while the card rides off-screen.
  //   1 â†’ 2  the next name rises from BELOW and pushes "×œ× ×¢×›×©×™×•" up and out,
  ...
  // very first frame of a pull is already tracked (no catch-up jump) â€” in
  ...
  // "×œ× ×¢×›×©×™×•" and back) went with the TabStrip on 2026-07-19, so nothing
  // renders `skipSlide` any more. It is kept because its withTiming completion
  // callbacks are what advance the phase machine below â€” it is now a clock for
  ...
  // edge on the pause-icon centre â€” consumed by the first-time skip tutorial.
  // Measured from the home pane's height (see the empty-pane onLayout).
  const tutorialPeekV = useSharedValue(0)

  // â”€â”€ Pull-to-skip (page1) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  // The whole pull behaviour â€” gesture, 1:1 resistance, half-screen commit,
  // the ride-off-screen animation, AND the first-time tutorial choreography
  // + once-ever trigger â€” lives in usePullBehavior. runIgnore is the commit
  ...
  const runIgnore = useCallback(() => {
    ...
    // the card is put away with app/pause instead â€” page1 goes `locked`, which
  ...
  // â”€â”€ WHAT A PULL DOES, PER STATE â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  // (user directive 2026-08-01.) The card is draggable in EVERY state it is on
  // screen in, not only while watching â€” a surface the app puts in front of you
  // is put away the same way wherever it came from. What differs is what the
  // release does, and it splits in two:
  //
  //  Â· The states that ASK first â€” waiting on an invitation I sent, and a chat â€”
  ...
  //  Â· The states that simply END â€” watching (skip) and a card that is already
  //    over (continue) â€” ride off ('slideOff') and fire, with no popup at all:
  ...
  // Sync remote â†’ displayed. The card only rises (mounts with SlideInDown)
  ...
  useEffect(() => {
    ...
    // covers is never promoted â€” and it is RELEASED rather than merely hidden,
    ...
    // Same user already on screen â€” this run was triggered by a state-only
    // change (e.g. watchingâ†’waiting after invite). Skipping the preload is
    ...
    // setDisplayedMatch(null) â€” e.g. runCancel â€” flips that condition true,
    ...
    // clears it: the visible "downâ†’upâ†’down" on cancel.
    ...
    // A match just started â€” swap the card NOW, without the preload hold.
    ...
    // prefetch shares expo-image's disk-cache key â€” single source of truth in
    // matchImageUrls.
    const urls = matchImageUrls(remoteMatch)
    if (urls.length === 0) {
      // No photos to wait for â€” promote immediately. No image-loading wait,
      ...
      // Server has answered with a candidate; from here until the warm-up
      // reports every photo painted we're loading this profile's images â€” swap
      // the scanning headline to "Loading profile data". The warm-up itself
      // (disk-cache, then paint into a hidden card) is WarmCard's, shared with
      // the incoming invitation; page1 only says WHICH candidate is being
      // warmed and what happens when it lands (onPreloadReady).
  ...
  // the same card read again after a relaunch â€” page1 keeps its occupant â€” is
  ...
  // signal to promote the match into the visible slot â€” at that point the
  ...
  const onPreloadReady = useCallback((readyUserId: string) => {
    // Paused mid-skip â€” never promote (defensive; runPauseFromSkip also nulls
    ...
    // Reset the pull transform first â€” pull-to-skip leaves pullY at screenH
  ...
  const isMatchCardOpen =
    ...
    isEndedState
  // â”€â”€ Match-state actions â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  // costing while it stands, and offers the one thing there is to do about it â€”
  // so "Cancel invitation" FIRES on the tap. It used to close and chain to a
  // `ConfirmDialog` that repeated the same words ("Cancel invitation" â†’
  ...
  // Which board's message is up, if any â€” see "what the heading chip's mark
  ...
  // raises ANOTHER popup has to chain through `onClosed` â€” iOS silently refuses
  // to present a second Modal while the first is still up â€” and this is the same
  ...
  // Report is NOT here â€” it moved to a dedicated flag button on every match
  ...
  // report confirm is driven off this â€” the surface is detected server-side,
  ...
  // match card, and it holds EVERYTHING the pair shares â€” every mutual friend
  ...
  // (components/SharedListPopup.tsx â†’ useSharedCircles).
  //
  ...
  const runAction = (
    endpoint: string,
    key: string,
    /** `ok` is whether the server took the action: the RPC's own refusals come
     *  back 400 (app/index.ts), so a caller that has put something on the screen
     *  for the answer knows to take it down again. */
    ...
    setBusy(true)
    setPendingKey(key)
    const done = (ok: boolean) => { setBusy(false); setPendingKey(null); onDone?.(ok) }
  ...
  // promote paths (the remoteâ†’displayed sync effect and onPreloadReady) and the
  // radar / warm-up / loading copy are torn down â€” dropping `preloadingMatch`
  // unmounts the WarmCard, so a candidate mid-warm never lands either.
  ...
  // runFind's .then). No confirm dialog â€” the user asked for an immediate
  ...
  const runPauseFromSkip = useCallback(() => {
    ...
    after
      ...
      // outlive the pause â€” a stuck latch + a stale page1 'watching' would
  ...
  // the sync effect's clearLoading path), cancel has no follow-up search â€”
  ...
  const runCancel = useCallback(() => {
    ...
    // THE POPUP IS CLOSED BY THE REQUEST LANDING, NEVER BY THE TAP (user
    // directive 2026-08-02). It used to close here, on the tap, and drop the
    // card with it â€” so the invitation read as cancelled a whole round trip
    // before the server had said so, and a refusal or a dropped invoke left the
    // user looking at a board that had never changed. The surface the decision
    // was taken on stays up with the button's spinner in it, and the card goes
    // when the answer does. (The message popup is the ONE way here: the chip's
    // mark and the drag both raise it.)
    invoke('app/cancel', {})
      .then(() => {
        setBusy(false)
        setPendingKey(null)
      ...
      .catch(err => {
        ...
        setBusy(false)
        setPendingKey(null)
        // No chat, no cancel: the popup stays where it was so the user can try
        // again, and the card under it is untouched.
      })
  }, [busy])

  // â”€â”€ Declining a pending invitation â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  // need the question named, because neither of them said "decline" â€” that is
  ...
  // surface the user OPENED â€” it is the app asking him something, and the two
  ...
  // AFTER this Modal is gone â€” two Modals presenting at once on the same stack
  ...
  // its own â€” no keep-alive timer needed the way the inline footer once was.
  ...
  // a candidate landed in `watching` â€” the user pressed play, a profile came
  // up, and the invite sheet appeared with no tap on the card's round action
  // (bug, 2026-07-28). Clear it the moment it stops being showable, so that
  ...
  // THE SPINNER RUNS FOR AS LONG AS THE POPUP DOES (user directive 2026-08-02).
  // Sending is the one action here that is not closed by its own request: the
  // server's answer lands, then Realtime flips page1 to `waiting`, and the popup
  // slides away on that. `busy` drops at the first of those, so for the gap
  // between them the user was looking at a live-reading Send button over an
  // invitation the server had already taken — press it again and the second
  // request is refused. This holds the spinner across the whole of it, exactly
  // as `answeredInvite` holds the accept's. Released only where the popup itself
  // ends: it stops being showable (the flip that closes it), or the request came
  // back a refusal, when there is no flip coming at all.
  const [inviteSending, setInviteSending] = useState(false)
  useEffect(() => {
    if (inviteSending && !invitePopupVisible) setInviteSending(false)
  }, [inviteSending, invitePopupVisible])

  // â”€â”€ Invitation lapse â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  // The clock reaching 00:00 is the only local signal that an invitation died â€”
  ...
  // state comes back over Realtime â€” which is what swaps the card. Nothing is
  ...
  // 2026-08-01). It was a status card at the top of the page2 card â€” a title, a
  // paragraph and two buttons â€” and it is one line and two chips now: the quiet
  ...
  //
  // Being unaffordable does not dim it â€” the press routes to the buy popup
  ...
  // one user this was written for â€” someone waiting on an invitation of his own,
  // with his only credit riding it â€” to a buy popup instead of to the chat.
  // (built below, beside page1's, where the ended-state predicates are)

  // Hero-photo overlay button on the page1 MatchCard:
  //   - watching â†’ the CHAT glyph (user directive 2026-08-02, replacing the
  //     heart this button wore since the card existed). Both sides of an
  //     invitation carry the same mark now, for the reason the receiving side
  //     was given it on 2026-08-01: a heart says "this person", and what an
  //     invitation offers â€” sent or answered â€” is the conversation. Tapping it
  //     opens the invite popup (openInvitePopup); the old
  //     scroll-to-inline-footer behaviour is gone with the footer. Pause is NOT
  //     on the card any more (2026-05-22, user request) â€” the only game-mode
  //     pause control is the home pane's center.
  //   - chat     â†’ OPEN CHAT. This used to be the end-chat X; that moved into
        ...
        ? [
            {
              key: 'like',
              icon: <ChatIcon color={INK} size={ICON.huge} />,
              // The mark no longer scrolls to an inline invite footer; it
  ...
  // ConfirmDialog popup â€” the inline centerNotice below owns the icon/text
  ...
  // find" â€” when state is `free` we surface the no-one-nearby + Search Preferences UI.
  // Never offer the find/play button while geo-gated â€” the server returns no
  ...
  // permission â€” the chat and invite overlays are unreachable. The MENU is
  ...
  useEffect(() => {
    if (!overlaysGated) return
    setOverlays(prev => prev.filter(o => o !== 'chat'))
    // Nothing is going to rise over home now, so nothing will announce that the
    // answered card is covered: let it go (see `answeredInvite`). A card kept
    // for a cover that never comes is a resolved invitation sitting on the
    // screen for as long as the gate lasts.
    setAnsweredInvite(null)
  }, [overlaysGated])

  // â”€â”€ Queued find â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  // swallowed â€” the reported "first tap doesn't always work". Now the tap is
  ...
  useEffect(() => {
    ...
    // flipped) â†’ the queued find is moot, drop it.
  ...
  // the user WHY the access is required. Notif is gendered (×ª×¤×¡×¤×¡ / ×ª×¤×¡×¤×¡×™);
  ...
  // or the server gate: not-in-any-group â†’ request-to-join / waiting, or
  ...
  // matches the old isPermMode precedence (permission first â€” you must fix
  ...
  // photo and a tap opens the profile-preview sheet â€” the only useful action
    ...
    isPermMode
      ...
      : geoGated
        ? (availability?.reason === 'push'
            ...
            // server-side token. Stays a bell icon â€” the call to action is
            ...
            : availability?.state === 'not_yet'
              ...
              // other geo-gated branches â€” show the user's avatar, tap opens
              ...
              // groups) â€” render the user's own avatar; tap opens the profile
        ...
        // directive 2026-08-01). A tap goes straight to the photo step â€” the
        ...
        // build-a-profile glyph â€” the circle wears what it opens.
        : browse.spent
          ? {
  ...
  // â”€â”€ Invitation countdowns â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  // the status card that announces the invitation â€” a block at the TOP of the
  ...
  // outgoing (this user invited someone and is waiting) â†’ InviteTimerCard;
  // incoming (someone invited this user) â†’ ReplyingInviteCard.
  //
  ...
  // old arrangement wrong was never the words â€” it was that scrolling to the face
  ...
  // Realtime events behind it, the sheet's rise â€” so the one thing between the
  // press and the conversation is the app plainly working on it, and there is
  // never a live accept button on a card the user has already answered. Both go
  // at the same moment, and it is the chat's own arrival that says when.
  const chatCovered = useCallback(() => {
    setAnsweredInvite(null)
  ...
  const answerInvite = useCallback(() => {
    ...
    runAction('app/approve', 'replying-accept', ok => {
      ...
      // here instead â€” the server's own answer has already replaced the card.
      if (!ok) chatCovered()
    })
    // eslint-disable-next-line react-hooks/exhaustive-deps
  ...
  // "this person" and what accepting actually gives is the conversation â€” and
  // since 2026-08-02 the SENDING side wears that same glyph for the same
  // reason, so the two are now one mark. It carries NO credit gem, exactly as
  // the sending side does not: what the credit costs is the POPUP's to say, in
  // words, which is where a sender reads it too.
  //
  ...
  // so it carries this card's random skip line (SKIP_HEADLINES) â€” invisible
  ...
  // the server, so it is up for exactly as long as page2 carries a card. That is
  // what makes it impossible to desync, and why an incoming invitation takes
  // visual priority over whoever the user is currently watching â€” it paints
  ...
  // invitation is actually about â€” and it used to be handed over empty: the
  // sheet went up the instant `relations` carried a page2 card, so the first
  // photo landed somewhere in the middle of the entrance, the second and third
  // after it had stopped, each one decoding on the very frames the slide-up
  // needed. So the whole card is painted out of sight first (WarmCard: the
  // photos to disk, then into a hidden copy of this card at this card's size)
  // and the sheet opens on the far side of that â€” its images come off the
  // memory cache on the first frame, and nothing arrives during the animation
  // because everything has already arrived.
  //
  // Held by USER ID, never by the card object: page2's card is rewritten by
  // every state it passes through (pending â†’ answered â†’ dead, a refreshed
  // last_seen), and re-warming on any of those would put the sheet away and
  // raise it again for a face already on the screen.
  const [invitePaintedId, setInvitePaintedId] = useState<string | null>(null)
  const inviteCardId = inviteCardMatch?.user_id ?? null
  useEffect(() => {
    // The invitation resolved. Forget it, so the same person inviting again
    // later is warmed again rather than trusted from a card that has been gone
    // for an hour.
    if (!inviteCardId) setInvitePaintedId(null)
  }, [inviteCardId])
  const inviteWarming = inviteCardMatch && inviteCardMatch.user_id !== invitePaintedId
    ? inviteCardMatch
    : null
  // The gate is NOT a reason to hold the warm-up back: while `overlaysGated` the
  // photos load anyway, so the moment the gate lifts the card is already there.
  const inviteOverlayOpen = !overlaysGated && !!inviteCardMatch && !inviteWarming
  // The sheet's onClose. Only the DEAD card can reach it now â€” a pending
  // invitation is not swipeable and wears no X (see openRefuseConfirm above) â€”
  ...
  // What a committed pull on the page1 card does â€” the handler behind
  ...
  // the card's MESSAGE â€” the state's title, what the invitation is costing while
  // it stands, and the one thing to do about it â€” instead of a confirm dialog of
  ...
  // `profile?.â€¦`, so a null profile renders a bare INK screen: no card, no
  // play button, no chrome, and no way out. That is the reported
  // blank-screen-on-launch â€” a device holding a dead session read the profile
  ...
  return (
    <View style={styles.backdrop}>
      {/* Glyph colour only. The bar BACKGROUND can't be set under Android
          edge-to-edge (see AppStatusBar) â€” the paused "muted" tone comes from
          the page behind the band, not from a status-bar colour. */}
      <AppStatusBar style="light" />
      <View style={styles.shell}>
            {/* page1 â€” the only standalone screen. The shell-wide sideways pan
            ...
            <View style={{ flex: 1 }}>
              <View style={styles.root}>
                ...
                <View style={styles.page1}>
                  {/* Empty / no-match pane â€” always mounted underneath. The
                  ...
                  >
                    {/* Empty pane â€” centers the headline+avatar group in the
                        ...
                        ready-to-find / no-one-nearby / "×œ× ×¢×›×©×™×•" during
                    ...
                    <View style={styles.permScreen}>
                      {/* THE BAND: from where the topbar ends down to the top of
                          the dock â€” i.e. the whole of page1. The circle is
                      ...
                      <View
                        ...
                        onLayout={e => {
                          // The circle's centre in the pane's own Y space â€”
                        ...
                        <View style={styles.permAvatarWrap}>
                          ...
                          <Pressable
                            onPress={() => {
                              ...
                              if (isReadyToFind) {
                                ...
                                // radar rings cover the wait â€” no spinner).
                                requestFind()
                                return
                              }
                              if (page1Profile) {
                              ...
                              // circle can do is open the SEARCH PREFERENCES â€”
                          ...
                          >
                            ...
                            {centerNotice ? (
                              ...
                              // mark â€” the same magnifying glass the dock's
                      ...
                      {/* And the drawing is centred in what is left BELOW the
                          circle â€” the mirror of the headline band above it, so
                  ...
                  {/* The invitation being painted where nobody can see it, so
                      that the card below can rise complete. It stands in page1's
                      own box, which is the box that sheet rises in, so the
                      photos decode at exactly the size they will be drawn at. */}
                  {inviteWarming ? (
                    <WarmCard
                      match={inviteWarming}
                      viewerLocationType={resolveLocationType(profile)}
                      cardHeight={paneHeight}
                      onPainted={setInvitePaintedId}
                    />
                  ) : null}
                  ...
                  {/* THE OTHER GAME CARD, IN THE SAME BOX AS THE FIRST (user
                      ...
                      second rises exactly where the first does â€” inside page1,
                      clipped at the dock's top edge â€” instead of taking the whole
                      ...
                      directive 2026-07-30) â€” so while one is up this card has
                      neither of the two ways out a surface normally has:
                        ...
                        swipeToClose  off, so the card does not move under the
                                      ...
                                      gesture a third route to the same decision â€”
                  ...
                  <OverlaySheet
                    ...
                    // springs it home and raises the decline confirm â€” a named,
                ...
                {/* THE DOCK. Under page1, in the flow, so it is the one thing on
                    this screen a match card is never laid over â€” see the note on
                ...
                <BottomSheet
                  ...
                  // `answeredInvite` too: an accept taken here is held until the
                  // chat is standing over it, which is well past its own request,
                  // and this popup is one of the two surfaces that hold for it.
                  busy={busy || !!answeredInvite}
                  onClosed={() => {
                ...
                >
                  {(messageOpen === 'page2' || messageOpen === 'approve') && messageInvite ? (
                    <ReplyingInviteCard
                      ...
                      // that popup is the approval and what it costs â€” not a
                      ...
                      // was written for â€” someone waiting on an invitation of his
                      // own, with his only credit riding it â€” to a buy popup
                      // instead of to the chat.
                      affordable={approveFunds >= CREDIT_COST.approve}
                      // The accept holds BOTH this popup and the card under it
                      // until the chat is standing over them (see `answerInvite`
                      // / `chatCovered`), so the whole of the transition is one
                      // spinner and then a conversation. The request landing is
                      // deliberately NOT what closes this one â€” the server's
                      // answer arrives a second before the screen is ready for
                      // it, and this popup is what covers that second.
                      onAccept={answerInvite}
                      // AND IT FIRES (user directive 2026-08-02, the same rule
                      // that turned the waiting card's Cancel): this popup is
                      // already the whole question â€” who invited me, what
                      // approving does and what it costs â€” so the button beside
                      ...
                      // The spinner runs for as long as the ANSWER does, which
                      // outlasts the request: `answeredInvite` is only let go
                      // when the conversation is on the screen.
                      busy={busy || !!answeredInvite}
                      acceptLoading={(busy && pendingKey === 'replying-accept') || !!answeredInvite}
                      inPopup
                    />
                    ...
                    <CardMessage
                      ...
                      // is the question â€” it says what the invitation is doing
                      // and what cancelling forfeits â€” so the button is the
                    ...
                    <CardMessage
                      ...
                      // TAP (user directive 2026-08-02): an action that rewrites
                      // the user's state holds the surface it was taken on, with
                ...
                {/* Invite prompt popup â€” raised by the hero heart in the
                ...
                <BottomSheet
                  ...
                  // `inviteSending` too: the send outlasts its own request, and
                  // a popup that cannot be tapped away must not be draggable away
                  // either.
                  busy={busy || inviteSending}
                  onClosed={() => {
                ...
                >
                  <ReplyingInviteCard
                    ...
                    onAccept={() => {
                      ...
                      // flips state â†’ 'waiting', which slides it away.
                      setInviteSending(true)
                      runAction('app/invite', 'invite-confirm', ok => { if (!ok) setInviteSending(false) })
                    }}
                    ...
                    busy={busy || inviteSending}
                    acceptLoading={(busy && pendingKey === 'invite-confirm') || inviteSending}
                ...
                {/* Ending the chat â€” opened by "End chat" on the card's heading
                    ...
                    has nothing further to ask â€” what it owes the user is what the
                ...
                >
                  ...
                  <SheetActionPair
                    options={[{
                      ...
                      // `busy` guard and nothing else â€” `disabled` here would
                ...
                {/* Everything we share, opened by tapping a card's circle chip.
                    ...
                    (user directive 2026-07-29) â€” no My-Friends roster under it,
              ...
              {/* The floating menu button is GONE (user directive 2026-07-30),
                  ...
                  carry â€” which rides the Circles entry, the one that actually
                  leads to the queue it counts. Home's top-START corner belongs to
                  nothing: do not put a control back there. */}
            </View>

        {/* â”€â”€ Overlays, painted low â†’ high â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
            ...
            of them any more â€” it is a profile the GAME hands the user, so it
        ...
        {/* Chat. dragFrom='header' is REQUIRED: the message list is an
            ...
            reopening is instant with no reload â€” a close/reopen no longer
        ...
        <OverlaySheet
          ...
          // conversation did not already say. The header ROW stays â€” with
        ...
        {/* (The MENU drawer stood here. It, the page it carried and the button
            that opened it are deleted â€” user directive 2026-07-30. Everything it
            held is home's dock and the two popups the dock raises, so nothing
            about the app is reachable only from a drawer any more.) */}

        {/* Profile preview, opened from the dock's first entry â€” swiping it down
            returns to home.
            ...
            photos, and the one mark over it â€” the heading tile's plus â€” is
        ...
        {/* Circles hub, stacked ON TOP of the menu it was opened from â€”
...
const styles = StyleSheet.create({
  ...
  // own â€” otherwise its shadow would sit over the photo.
  overlayCardBare: {
    backgroundColor: 'transparent',
    shadowOpacity: 0,
    elevation: 0,
  ...
  // pulled card off the strip â€” see the note at the call site.
  page1: {
    flex: 1,
    overflow: 'hidden',
  },

  // (The hidden preloader's box stood here â€” absoluteFill at opacity 0, the
  // card painted where nobody can see it. It is WarmCard's own now, stated once
  // for both the next candidate and the incoming invitation.)
  // Outer Animated.View â€” fills the pane so the photo can flex to take all
  ...
  // â”€â”€ Permission screen (no card) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  permScreen: {
    flex: 1,
  },
  ...
  // top edge. Both are layout, so the button's touch target moves with it â€” a
  ...
  // top of the dock â€” the whole of page1, which is exactly what this pane is.
  //
  ...
  // what is left BELOW it â€” one band each, mirrored on the same middle line.
  ...
  // circle, and from the circle to the art, is whatever centring leaves â€” which
  ...
  // action surface. No background â€” the image covers the full circle.
  permAvatarImage: {
    width: '100%',
    height: '100%',
  },
