...
- `cancel`: `A.page1` → `locked` (no message — A initiated). `B.page2` → `locked` + `message = 'cancel'`, profile preserved so B sees who cancelled. B clears via `clear2`. **Forfeits A's held heart** (`app_cancel` calls `_credits_clear_hold` for `_credits_cost('invite')` — the heart A spent on send stays in the system; see "Credits economy → Hold + refund / forfeit").
...
A per-user credit wallet at `relations.credits` gates the spend-y actions. State shape: `{balance:int, tier:'free'|'pro', held:int, granted_on:'YYYY-MM-DD'|null, next_grant_at:timestamptz|null}`. It is a sibling of `last_add_at`/`availability` (economy state, not profile data) and is mutated atomically inside the same `SELECT … FOR UPDATE` transaction as the state transition that spends it. Credits are private (never embedded by `make_profile`, so they don't leak into other users' snapshots). `held` reserves balance against a live waiting invite — see "Hold + refund / forfeit" below.
...
**Costs (SQL `_credits_cost`):** `approve` 1, `broadcast` 1, `invite` 1, `cancel` 0. **Sending an invite costs 1 heart**, held server-side until the invite ends (see "Hold + refund / forfeit"). `app_invite` enforces a credit precondition under the FOR-UPDATE lock: `_credits_balance < _credits_cost('invite')` → `{error:'no_credits'}` (HTTP 400), no state change. **Cancelling a sent invite is free in additional cost** but is the only exit that does NOT refund the held heart — it forfeits it (the heart "stays in the system"). The mobile client mirrors the precondition: the page1 "send invite" CTA is rendered `disabled` (no-op on press, no explainer popup) when `starsBalance < CREDIT_COST.invite`; the waiting-card cancel button carries no cost badge and is never disabled-on-balance. (This **reverses** the 2026-05-22 model where cancelling cost 1 and inviting was free — user request 2026-05-31, migration `restore_invite_credit_hold`. The brief "cancel costs a heart" era is gone; the heart-spent-on-send / cancel-forfeits model is restored.) The action costs (and the `3`/`10` tier amounts) are mirrored in places that MUST stay in sync: the SQL `_credits_*` helpers (source of truth + enforcement), `mobile/src/lib/credits.ts` (`CREDIT_COST`, display only), and the `defaultRelations` seed in `supabase/functions/user.ts` (`balance:3, held:0`). Change them together. (Same discipline as the 30-minute broadcast window.)
...
**Hold + refund / forfeit (restored 2026-05-31, migration `restore_invite_credit_hold`).** Sending an invite reserves the inviter's 1 heart on the inviter's row (`balance -= 1`, `held += 1`) via `_credits_hold(rel, amount)`. The reserved heart's fate depends on how the invite ends:
- **Every non-cancel exit refunds it** (`held -= 1`, `balance += 1`, balance capped at the tier cap so a daily grant in between can't overflow): `app_decline` on the invitee side, `app_approve` on the inviter (B) side, `app_expire_sweep` in BOTH passes (inviter-driven and invitee-driven catch-all), `_kick_page1_at` when it transitions a `waiting` user to locked (matched elsewhere / kicked by mutual-match propagation), the mutual-match branch of `app_invite` (the *target's* held heart is refunded — A had no hold to refund), `app_logout_cleanup` (refunds anything still in `held`), and both admin release RPCs (`app_admin_release_page1` when releasing a waiting page1, `app_admin_release_page2` when releasing a pending page2). The invariant: at locked / locked rest state, `held = 0`.
- **`app_cancel` forfeits it** via `_credits_clear_hold(rel, _credits_cost('invite'))`: `held -= 1`, balance unchanged. The heart is the user-visible cost of cancelling. `app_cancel` carries NO additional credit precondition (the spend already happened on send) and never charges 1 on top.

Helpers: `_credits_hold(rel, amount)` (defensive `GREATEST(0, …)` clamps on both sides; caller MUST precondition on `balance >= amount`), `_credits_refund(rel, amount)` (balance capped at the tier's cap via `_credits_tier_cfg`), `_credits_clear_hold(rel, amount)` (held only). `_credits_held(rel)` reads `held` (0 default). `_credits_reset_to_cap` preserves `held` (used only by `app_set_tier` free→Pro, where the user might be mid-invite). The hold/refund/forfeit shape mirrors the original pre-2026-05-22 machinery; the 2026-05-22 "inviting is free / cancel costs 1" model is dead.
...
**Preconditions / failure landings.** `app_invite` checks `_credits_balance < _credits_cost('invite')` under the FOR-UPDATE lock and returns `{error:'no_credits'}` (HTTP 400) with **no state change** (the user stays in `watching`); the `page1=locked,message=invite` (`invite-fail`) landing remains for non-credit invite failures (target page2 not free, target mismatch under lock, etc.). `app_cancel` has **no credit precondition** (the heart was spent on send; cancelling forfeits it). `app_approve` `balance < v_approve_cost` → `page2=locked,message=approve` (`approve-fail`); `app_add` `balance < 1` → `{error:'no_credits'}` (HTTP 400). No new push codes.

**Relations-rebuild RPCs preserve credits.** `app_approve` (the inviter B side), `app_resume`, and `app_logout_cleanup` rebuild `relations` with `jsonb_build_object` and need to carry `credits` forward; `app_approve` (B) and `app_logout_cleanup` actively mutate `credits` (refund B's held heart / refund anything still held respectively), and `app_resume` carries it forward via `_credits_ensure`. `app_pause` no longer rebuilds `relations` at all (page1-only `jsonb_set`, see "Relations-rebuild RPCs also preserve the gate keys"), so `credits.held` survives by construction. New users are seeded `{balance:3, tier:'free', held:0}` by `user.ts`; `granted_on`/`next_grant_at` fill on the next cron tick (≤60s); `_credits_ensure` defensively seeds any credit-touching RPC that meets a credit-less row (and patches a missing `held: 0` onto pre-2026-05-31 wallets without rebuilding the whole subtree).
...
A 6-digit code per group that managers / admins share verbally. The recipient installs the app via the generic install link (no per-group deep link) and enters the code either as the final onboarding step or from the settings "My groups" sheet — joining is **additive** (a user may be in any number of groups), and the operation is idempotent.
...
Mobile (onboarding):
- Step 6 of onboarding is an **optional** invite-code screen with a 6-digit input (single line, monospaced, `number-pad` keyboard, `INVITE_CODE_LEN = 6` shared via `mobile/src/lib/groups.ts`). The Continue button shape-shifts: label `"Skip"` when the input is empty, `"Join"` when 6 digits are entered (partial input keeps it disabled). Bio is held in local state across steps 5→6 and submitted only after step 6 resolves (skip or redeem) — so a relaunch of the app mid-flow re-enters onboarding from step 5 instead of routing to home with no group attached. On invalid code the error renders inline; bio is not submitted, the user can retry or skip.
- `finishOnboarding` order: redeem (if a code was entered) → flush photo uploads → `/app/profile { bio }` → `useUserStore.update({ bio })`, which is what `_layout.tsx` watches to redirect to `/home`. Doing redeem first means the bio save's auto-find runs server-side after the membership is already in place.
...
- The group's `enabled` column was already dropped (see "`groups.enabled` was REMOVED 2026-05-25") — the redeem RPC does not filter by it. Every group is joinable.
...
- `public.user_availability(uid uuid, loc geography) → jsonb` — **single source of truth** for a user's effective availability. **Live precedence (2026-05-31, after geo-gate restoration — migration `restore_geo_gate`):**
  ...
  2. `group_blocked(uid)` ⇒ `{state:'unavailable', reason:'group'}` — TRUE iff the user has ≥1 group AND none of them is enabled. A user in 0 groups is NOT group_blocked (the existential predicate is false on an empty set), so they stay available — preserving the no-group escape hatch.
  3. `area_state(loc)`:
...
Every site that persisted availability into `relations.availability` still calls `user_availability(user_id, location)` — same single helper. `others()` carries the `push_blocked` + `group_blocked` + `area_available` candidacy clauses + the preserved credits clauses. Admin geo mutations (`createArea` / `updateArea` / `deleteArea` / `setAreaMode`) and group mutations (`setGroupEnabled` / `setUserGroupAssignment` / `deleteGroup`) all fire `triggerResync` so a flip propagates to every affected user immediately. The `/functions/v1/ext/resync` endpoint and the per-minute cron `app_area_resync` are the safety net for any transition the synchronous push doesn't cover (push-presence flips Expo doesn't deliver synchronously, plus the eventual `not_yet`/group-enable / scheduled-area-launch race conditions).

**Group-membership gate — REINTRODUCED 2026-05-26** (migration `20260526020000_restore_group_disable_gate`). After a brief stint (2026-05-25 → 2026-05-26) where group membership was purely organisational, the admin gate is back: a user whose **all** group memberships are disabled becomes `unavailable`. A user in **0 groups** stays `available` (the no-group escape hatch from 2026-05-25 is preserved). A user in ≥1 enabled group is `available` regardless of any other disabled groups they hold. Concrete rules:
- 0 group memberships → **available** (no-group escape hatch; new signups land here since auto-join was retired 2026-05-25).
- ≥1 enabled group → **available** (the enabled group dominates any disabled siblings).
- ≥1 group AND every one of them disabled → **unavailable** (`reason:'group'`).

Wired through:
- `public.groups.enabled boolean not null default true` — the per-group flag.
- `public.in_enabled_group(uid uuid) → boolean` — "holds ≥1 enabled group" (admin facets / dashboard tiles can read it).
- `public.group_blocked(uid uuid) → boolean` — the gate predicate (`has ≥1 group` AND `NOT in_enabled_group`).
- `public.user_availability` precedence (above) consults `group_blocked` between `push_blocked` and the `available` fallback.
- `public.others()` carries an `AND (NOT only_available OR NOT public.group_blocked(other.user_id))` candidacy clause — a `group_blocked` candidate is dropped from everyone's pool. No-op when no groups are disabled (the typical post-launch state).
- Web admin: `/groups` list (per-tile status badge + Enable/Disable button) and `/groups/[groupId]` (status badge + Enable/Disable button in the header) flip the column via the `setGroupEnabled` server action. The action fires `triggerResync` so the gate change propagates to every affected user immediately. `setUserGroupAssignment` also fires `triggerResync` again (membership is a gate input).
- The mobile gate UI (`home.tsx`) is unchanged: an `unavailable` user sees the static `home.geoGate.unavailable` headline, the side tab is removed, find/play is suppressed. With geo restored as a gate (2026-05-31) the gate-copy reading "not available in your area" is again literally correct for the dominant `reason:'geo'` case; for `reason:'group'`/`'push'` it remains observability-only (minor UX gap, not blocking).

What is **NOT** restored from the original 2026-05-19→2026-05-25 incarnation: the web admin's bulk enable/disable UI, the `?status=enabled|disabled` URL filter on `/groups`, the dashboard "disabled groups" / "role-gated users" facet tiles, the `?seg=role_gated` users-list segment, and the `admin_user_facet_counts` `role_gated` count. These are dead-but-revivable when the operator needs the lens. The current admin UX is the per-row toggle only.
