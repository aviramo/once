...
- `public.user_availability(uid uuid, loc geography) → jsonb` — **single source of truth** for a user's effective availability. **Live precedence (2026-05-26, after group-disable reintroduction):**
  ...
  3. Else ⇒ `{state:'available'}`.

  Geo areas (`area_state`/`area_available`) are still defined-but-unconsulted. The `in_enabled_group` / `group_blocked` helpers were restored in `20260526020000_restore_group_disable_gate`.

Every site that persisted availability into `relations.availability` still calls `user_availability(user_id, location)` — same single helper. `others()` carries the `push_blocked` + `group_blocked` candidacy clauses + the preserved credits clauses. **As of 2026-05-26 the gate is group-driven again** — admin group mutations (`setGroupEnabled` / `setUserGroupAssignment` / `deleteGroup`) fire `triggerResync` so a flip propagates to every affected user immediately. The `/functions/v1/ext/resync` endpoint and the per-minute cron `app_area_resync` are the safety net for any transition the synchronous push doesn't cover (push-presence flips Expo doesn't deliver synchronously, plus the eventual `not_yet`/group-enable race conditions).
...
- The mobile gate UI (`home.tsx`) is unchanged: an `unavailable` user sees the static `home.geoGate.unavailable` headline, the side tab is removed, find/play is suppressed. The mobile copy still reads as the geo "not available in your area" line (the `reason` is observability-only; minor UX gap, not blocking).
...
Helper SQL: `area_state(loc geography) → jsonb` (mode-driven; **defined but no longer consulted by the gate** — see "Group-membership gate"), `area_available(loc geography) → boolean` (likewise defined-but-unused by the gate), `group_blocked(uid uuid) → boolean` (**live** gate input — TRUE iff the user has ≥1 group AND none of them is enabled; see "Group-membership gate"), `push_blocked(uid uuid) → boolean` (positively-known no-notifications; see "Notification-presence gate"), `in_enabled_group(uid uuid) → boolean` (holds ≥1 **enabled** group — companion helper to `group_blocked`), `user_availability(uid uuid, loc geography) → jsonb` (the single source of truth: **(1)** push-block override, **(2)** group-block override, **(3)** else available; every availability writer goes through this), `app_availability(me_id uuid) → jsonb` (`{user, notify}` envelope), `app_area_launch_sweep() → jsonb` (`{processed, notify}`, cron). Note: `app_refresh_snapshots` does **not** recompute availability — location only changes via `start`/`location`/`focus`, and admin group/area changes propagate within ~60s via the next periodic `/app/location`, on app `focus`/launch, the per-minute `app_area_resync` safety net, plus the immediate `triggerResync` admin actions fire after every mutation.
