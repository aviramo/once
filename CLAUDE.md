...
## Keyboard avoidance for any text input in a popup/sheet (recurring bug — read this)

**Symptom the user keeps hitting:** ask for a text box in a popup, and the on-screen keyboard covers the field (you can't see what you type). This has recurred every time a `TextInput` is added to a sheet. It is now fixed structurally; do not reintroduce it.

**Root cause:** every popup in the app composes the single `mobile/src/components/BottomSheet.tsx`, which renders inside a React Native `Modal`. On Android a `Modal` gets **its own window** that does **not** receive the activity's `adjustResize` (even though `app.json` sets `softwareKeyboardLayoutMode: "resize"`), so the window never shrinks for the keyboard. RN's `KeyboardAvoidingView` is therefore a **no-op inside the Modal on Android** — the field stays hidden behind the keyboard. (`chat.tsx` hit the same wall and abandoned `KeyboardAvoidingView` for a manual keyboard-height lift.)

**The fix (already in place — use it, don't rebuild it):** `BottomSheet` has a `keyboardAvoiding?: boolean` prop. When true it listens to `Keyboard` will/did show+hide, computes the keyboard height from screen geometry (`screenH − endCoordinates.screenY`, robust to Gboard's clipboard strip), and lifts the whole sheet by that height via the existing animated `transform` (a `keyboardOffset` shared value subtracted from `translateY`). This works on **both** platforms. There is **no** `KeyboardAvoidingView` anywhere in `BottomSheet` — do not add one back.

**Rule for any future text field in a popup:**
- Never hand-roll a popup with a raw `Modal` + `KeyboardAvoidingView`. Compose `BottomSheet` (or a component that already does, e.g. `ConfirmDialog`).
- Any sheet whose body can contain a focusable `TextInput` **must** pass `keyboardAvoiding` to `BottomSheet`. `ConfirmDialog` does this automatically whenever its `noteInput` prop is set — so add text fields to a `ConfirmDialog` via `noteInput`, not by stuffing a bare `TextInput` into `description`.
- If you build a brand-new sheet composer, thread `keyboardAvoiding` through to `BottomSheet` the moment it has a text field. This is the single source of truth for keyboard avoidance; fixing/extending it there fixes every popup at once.
...
**Skip-latency optimization (migration `20260519130000_app_find_lookahead`).** Two composable wins remove the per-skip stalls without changing matching/viewer semantics:
- **Win 1 (client only):** the find/ignore HTTP response is the authoritative result of the actor's *own* action and already carries the next candidate in `relations.page1`. `invoke()` tags those two calls `'invoke:find'`; `userStore.applyServerUser` then trusts **only `page1`** from the response (merged over the last raw relations Realtime/fetch delivered, so `page2` / incoming-invite / watcher list stay Realtime-owned — they're only ever changed by *other* users' RPCs, which don't bump the actor's `last_seen` and so can't be ordered). This deletes the "wait for Realtime to redeliver the same state" leg. The rare page1-vs-broadcast cross-user race self-heals on the next Realtime tick (accepted "optimistic + quiet correction"). `lastRawRelations` (module-level in `userStore.ts`, reset in `clear()`) is the stash; the deriveCompat→legacy assembly is the shared `writeCompat()` helper. Old builds keep tagging `'invoke'` ⇒ byte-identical to before.
- **Win 2 (server `app_find`):** `app_find` now captures the **top-3** from one `others(me_row, true)` ranking (row 1 = the pick, unchanged; rows 2-3 = look-ahead) and ADDS a `lookahead` key (`Profile[]`, ≤2, fresh `make_profile`) to its `{user, notify}` envelope. The edge handler forwards `lookahead` **only for `find`/`ignore`** as a sibling key on the actor-row response body. The client (`api.ts`) fire-and-forget `Image.prefetch`es those via the shared `matchImageUrls()` helper (cache-key-exact with `MatchCard` — raw filename, NO `encodeURI`; `publicImageUrl` is wrong for this) so the user's *next* skip is a disk-cache hit. Look-ahead users are a transport-only hint — **NOT** registered as viewers (no `page2.profiles[]` write for them); only the actually-picked candidate is attached, exactly as before. Additive / not breaking: signature + `{user,notify}` contract unchanged, new key only, old clients ignore it, no `BACKWARD_COMPAT.md` entry (no shim). `app_ignore` inherits look-ahead for free (it delegates to `app_find`).
...
- **Pending (mobile, not yet implemented):** the unified `CenterNotice` UI (single component replacing the ~5 `ConfirmDialog`/`isPermMode` permission popups; the page1 `permCenterGroup` HeadlineArea-text + central Pressable becomes the per-reason icon-as-action-button; page2 non-destructive *display-hidden* notice mirroring page1's text — Q1 = display-hidden, NOT `app_lock2`; loc-perm-loss → `POST /app/location {location:null}` for non-custom users). Server/edge/admin for this are done; the `home.tsx` integration point is the existing `styles.permCenterGroup` (HeadlineArea + the round Pressable that already renders play/avatar/`HamburgerIcon`).
