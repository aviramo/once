...
> **`groups.enabled` was REMOVED 2026-05-25** (migration `20260525050000_drop_group_disable`). Groups are now **purely organisational** — there is no "disable a group" concept. Every group is joinable; group membership does NOT affect availability. The web admin no longer surfaces enable/disable buttons or a status badge; the `setGroupsEnabled` server action and `in_enabled_group` / `group_blocked` SQL helpers are deleted. Deleting a group now detaches every member from it first (the user accounts stay), so the old "cannot delete a group with members" restriction is gone.
...
- `public.user_availability(uid uuid, loc geography) → jsonb` — **single source of truth** for a user's effective availability. **Live precedence (2026-05-25, after group-disable removal):**
  1. `push_blocked(uid)` ⇒ `{state:'unavailable', reason:'push'}` (the only gate left).
  2. Else ⇒ `{state:'available'}`.

  Geo areas (`area_state`/`area_available`) and group membership are not consulted. The `in_enabled_group` / `group_blocked` / `role_blocked` / `area_*` helpers were either dropped (group ones) or are defined-but-unused (area ones).

Every site that persisted availability into `relations.availability` still calls `user_availability(user_id, location)` — same single helper, simpler body. `others()` carries only the `push_blocked` candidacy clause + the preserved credits clauses. Admin group mutations no longer call `triggerResync` (membership doesn't affect availability any more, so the resync would be wasted work). The `/functions/v1/ext/resync` endpoint is still wired up and the cron still runs `app_area_resync` as a safety net for push-presence transitions Expo doesn't deliver synchronously.

**Group-membership gate — REMOVED 2026-05-25** (migration `20260525050000_drop_group_disable`). Groups are now purely organisational: assigning a user to a group does not gate them in or out of the app, and there is no "disable a group" concept. The web admin's enable/disable buttons, status badges, bulk enable/disable, the `?status=enabled|disabled` URL filter, and the dashboard "disabled groups" / "gated via group" tiles are all gone. The `setRoleEnabled` / `setGroupsEnabled` server actions and the `in_enabled_group` / `group_blocked` SQL helpers are dropped. Deleting a group now detaches every member from it first (the user accounts stay), so the old "cannot delete a group with members" restriction is gone too. The earlier "must be in ≥1 enabled group" / "in groups but all disabled ⇒ unavailable" rules (shipped 2026-05-19, refined 2026-05-25 to allow "no groups = available") are both **superseded**.
