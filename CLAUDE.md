...
### `permissions` (permission catalog) + `group_permissions` (group↔permission)

Admin-facing permissions module (2026-05-23, migration `permissions_module` `20260523010000`). **No dedicated screen** — permissions are attached to groups, managed from the per-group detail page (`/admin/roles/[groupId]`). The catalog itself is code-coupled (whoever consumes a permission key checks it in code), so new keys are added via migration, not from an admin UI. The web admin only manages the many-to-many assignment of cataloged permissions to groups.

`public.permissions`:

| Column | Type | Notes |
|---|---|---|
| `key` | text | primary key. Stable identifier (`'admin'`, `'owner'`). |
| `created_at` | timestamptz | default `now()` |
...
| Column | Type | Notes |
|---|---|---|
| `group_id` | uuid | not null, FK → `groups(id)` **ON DELETE CASCADE** (deleting a group cleans up its permissions) |
| `permission_key` | text | not null, FK → `permissions(key)` **ON DELETE CASCADE** (dropping a catalog key removes every assignment) |
| `created_at` | timestamptz | default `now()` |

Primary key `(group_id, permission_key)`. Index `group_permissions_perm_idx` on `(permission_key)`. RLS enabled, **no policies** on either table → service-role only (edge function + web admin), identical pattern to `areas` / `groups` / `reports`. Both tables are in the `supabase_realtime` publication so the admin's `RealtimeRefresh` listener (`tables="user_groups,groups,group_permissions"` on the group detail page) re-fetches when an assignment flips. Display labels live in the web i18n dictionary at `admin.permissions.labels.<key>` (fallback: render the raw key when no label is defined).

**Seed (migration `permissions_module`):** catalog seeded with `admin` + `owner`; `admin` attached to the existing `מנהלים` group; a new `בעלים` group is created (idempotent on name) and `owner` attached to it.

**`owner` permission gates the permissions UI itself (2026-05-24, migration `user_has_permission_helper` `20260524000000`).** User decision: *"בעלים הוא היחיד שיכול להעניק הרשאות או להסירן"*. So `setGroupPermission` (the only server action that writes `group_permissions`) is gated on `getOwnerUser`, not `getAdminUser` — an admin who is NOT in a group with the `owner` permission cannot grant/revoke any permission, including promoting themselves to owner. The group-detail page passes `isOwner` to `GroupPermissions`; non-owners see the same checklist read-only with an amber hint. Helper SQL function `public.user_has_permission(p_user_id uuid, p_key text) → boolean` (`sql stable security definer set search_path=''`, EXECUTE revoked from anon/authenticated, inlinable) drives the gate via the service-role admin client. Bootstrap: the same migration adds the single existing JWT admin (the only user who could sign into the web admin pre-2026-05-24) to the `בעלים` group so the new UI is operable on first use; new owners are added by toggling membership of the `בעלים` group.

**Admin gate is still JWT-based.** `getAdminUser` (`web/src/lib/admin-auth.ts`) still reads `app_metadata.role='admin'` — every other admin surface (dashboard, users, areas, reports, group-membership management) is unchanged. The owner gate is strictly stricter than admin, not a replacement; a holder of `owner` does NOT automatically also satisfy `getAdminUser` (and vice versa). Switching the admin gate to also read from `group_permissions.admin` is a possible follow-up; not done.
