...
- **Report rides the bottom-center of the LAST photo** (`reportOverlay` + `renderReportOverlay()` in `MatchCard.tsx`, moved off the top-END 2026-07-25): the same round primitive as the hero heart (full `ROUND_BUTTON_SIZE`, `ICON.huge` glyph), kept GREEN, horizontally centered, `overlayBottomOffset` off the photo's bottom edge so it sits level with the heart. It renders on whichever photo section is last (`lastPhotoKey`), so a single-photo profile carries it on the hero and a multi-photo one on the trailing photo — a card-level affordance on **every** match card, now read as "seen the whole profile, flag it". Omitted for the own-profile preview (no `onReport`). Top-START belongs to the shell (home's hamburger; a sheet's close X, which also sits at the START/leading edge); **top-END carries the card's name/age heading chip** (the `topEndFixed` column), with the solid-purple **"End Chat" chip** sitting beside it on the START side in chat state (`headingRow`, `headingAction` prop) — no other affordance may claim these corners.
- **On-photo chips are beige tiles that match the round buttons** (user directive 2026-07-25, reverses the earlier no-background treatment): every on-photo `Chip` is a solid `PHOTO_CHROME` (beige `SURFACE`) tile with the tone's own purple ink and the **same soft lift shadow the round buttons cast** — `LIFT_SHADOW` in `colors.ts`, spread by both `RoundButton` (`styles.shadow`) and `Chip` (`chipShadow`), so chips and buttons read as one fabric of beige tiles over the photo. Chip's old transparent white-ink-plus-glyph-shadow path (`chipOnPhoto`/`chipTextOnPhoto`/`glyphWrapShadow`/`PHOTO_INK_SHADOW*`) is gone. The **`solid` tone** keeps its full GREEN fill + white ink (chat "End"); **outlined** chips stay flat (no tile, no shadow). The **name/age heading** is pinned FIXED to the card's **top-END** (opposite the shell's hamburger), OUT of the scroll — the `topEndFixed` sibling column after the ScrollView, which also carries the own-profile add-chips beneath it. It is a normal-size chip with only **bold** text (`Chip bold`, not a bigger `size`). The bottom-START stack now holds only the fact chips: time/distance → kids/family → **shared group** (moved here from the bio 2026-07-25), each a beige tile, with the extra `marginBottom: LG` breathing room under the column (`infoLeft`). The **bio** is a big **beige chip** (`photoBioCard`, `backgroundColor: BG`, full `RADIUS`, padded) floated at the **bottom of the second photo**, inset on every side, so photos 1 and 2 stay flush with no bubble section between them. Its text is the purple `BIO_INK` (regular weight). Its END inset clears the floating heart (so it is never where the heart is) and its bottom inset lifts it above the report when that photo carries it. **The own-profile preview renders the exact same beige chip** (user directive 2026-07-25) — the inline bio editor (`BioField onPhoto`) lives INSIDE that chip, so a self card is pixel-identical to a match card; the `pointerEvents:'auto'` on the self chip is what lets the field catch taps. A plain white bubble (`aboutBubble`) survives only for a bio with **no second photo to sit on** (own-profile editor with a single photo, or a remote bio with one photo). Reels paging is on for the preview too, suppressed (`pagingEnabled = !bioFocused`) only while the bio field is focused so a keyboard-driven scroll isn't snapped to a page boundary. Bios are saved through `normalizeBio`, which strips blank lines (any newline run collapses to one).
