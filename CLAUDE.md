...
- Three tabs in pager-slot order: **icon-only Menu** (chrome, not a destination), `home.tabs.home` (always "Once"), `home.tabs.viewers` / `home.tabs.chat` (switches by `chatAvailable`).
- The Menu tab carries no text label — it's a single glyph (settings gear normally, `PauseIcon` when `gameModeOff`, `CloseBoldIcon` while the profile preview sheet is open). It has `flex: 0` (shrinks to glyph width with `paddingHorizontal: SM`) so the two content tabs absorb the freed flex and read wider. The two labeled tabs keep `flex: 1`. Mechanism: `TabSpec.label` is optional; an absent label triggers the compact tab style.
- Counts (unread chat messages, viewer counts) are **chained directly into the side-tab label** as `${label} ${n}` — there is no separate chip badge. One piece of text reads cleaner at small tab size and avoids the extra circle competing with the word. `TabSpec` has no `chip` field; callers build the full label string themselves. The label still pulses on alerting transitions (the pulse is applied to the label stack).
  - `chatAvailable && chatUnread > 0` → label = `${t('home.tabs.chat')} ${chatUnread}` (alerts on transition).
  - `page2PendingInvite` → label swaps to `home.tabs.invite` ("הזמנה") instead of `home.tabs.viewers`, and alerts on arrival via the tab-level pulse. The dedicated label alone signals an incoming invitation — no glyph or count.
  - `page2DeadInvite` → label swaps to the other user's name and, when the lock came from `expire`, the timer stays frozen at 00:00 under the label — together they communicate "this side is finished" without an extra pause icon, which read as noise next to the name.
  - else `watchers.length > 0` → label = `${t('home.tabs.viewers')} ${watchers.length}` (or `home.tabs.broadcast` while broadcasting).
...
| `data` | jsonb, default `{}` | Flat profile fields: `images: Image[]`, `bio?: string`, `family?: FamilyData` (`{hasKids, kids?, schedule?, isForKids?}`). Plus `weekStart`, `os`, `lang`, `appearance`, `push_token`, `role`, `location_custom`, `location_label`. Distance unit is no longer stored: the client derives it from device locale (`getLocales().regionCode`); see `mobile/src/lib/units.ts`. Legacy `data.units` may exist on rows written by older builds and is ignored. `location_custom: boolean` and `location_label: string \| null` track whether the user picked a manual address instead of the device's GPS; while `location_custom=true` the client suppresses location permission prompts and skips periodic GPS updates (the `location` column is whatever was last written from the manual pick). |
...
`location_custom` is only embedded when the snapshotted user has `data.location_custom = true` (manual-address mode); GPS mode omits the key. The mobile distance chip uses this to swap PinIcon → HomeIcon, signaling "distance is computed against a fixed address, not live GPS" so users understand why the value may not reflect actual proximity. The chip text also replaces "ממך"/"away" with the passive "מהמיקום שהוגדר"/"from the set location" — both swaps fire when **either side** is custom, since the viewer can't easily tell which side owns the home (and shouldn't need to).
