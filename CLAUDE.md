...
| Surface | `activation` | `commit` | header | notes |
|---|---|---|---|---|
| Menu (`SettingsPage`) | `sheet` | `dismiss` | bar, titled | never gated |
| Profile preview (`PreviewFieldPage`) | `sheet` | `dismiss` | floating | stacks on the menu |
| Chat (`ChatPage`) | `sheet` | `dismiss` | bar, titled | `dragFrom="header"` |
| Incoming invite | `scrollPan` | `confirm` | floating | swipe = decline request |
| Dead invite | `scrollPan` | `dismiss` | floating | close = `app/free2` |
...
- **Top-START on a card belongs to the shell, and nothing else may claim it.** The card's own chrome lives at top-END, in one row: the shared-group chip, then the report flag at the screen edge (`topEndOverlay` in `MatchCard.tsx`). The flag used to sit at top-start and collided with the hamburger; it spent one commit in the bottom action stack before landing here beside the chip (user decision 2026-07-19).
