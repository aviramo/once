...
- Container background is a **flat solid `PRIMARY`** (deep wine `#890824`) header — **no gradients anywhere and no drop-shadow** (the user explicitly removed both). `backgroundColor: PRIMARY`, seamless with the PRIMARY status bar (`StatusBar style="light" backgroundColor={PRIMARY}`); the *only* separation from the white content below is the deep-wine color contrast (no `boxShadow`, no elevation). The bottom edge is **square and full-width** (no `borderRadius`, no `overflow: 'hidden'`). The container carries a tight `paddingHorizontal: SM` (the user wants the row close to the screen edges); the chip↔glyph breathing room is a *separate* concern owned by `tabCompact.paddingHorizontal (LG) − TAB.indicatorInsetX` inside the row, so the edge margin and the inner padding tune independently. `paddingTop` is `topInset + XL` (not `MD`): the sub-label timer floats as a caption *above* the selected-tab chip, so the header needs real top breathing room to keep the timer clear of the status bar. Paused (`gameModeOff`) swaps the solid `PRIMARY` for flat `BLACK_MID`. There is intentionally **no conditional / scroll-driven header shadow** — it was built once and then removed at the user's request; do not reintroduce it.
- The "selected" indicator is **two layers**, both driven by the single `pagerProgress` swipe value so they track the finger 1:1:
  1. **ONE flat translucent-white chip** (`HEADER_PILL_FILL` solid `rgba` — *not* a gradient — plus `HEADER_PILL_BORDER` / `HEADER_PILL_SHADOW`), a single moving element (*not* per-tab opacity) that **spans the selected tab's full width** (minus `TAB.indicatorInsetX` each side) and **resizes** between the unequal tabs. **This is the state the user explicitly chose** (the original "measureInWindow" feel) — keep it; do not swap it for fixed-width / scaleX-per-settle / per-tab-opacity again. It is `position:absolute` inside a plain absolute-fill overlay (`styles.chipOverlay`, `pointerEvents="none"`, first child so it paints behind the labels), anchored physical **`left:0`**. `chipStyle` (a single `useAnimatedStyle`) animates per frame BOTH `width` (= interpolated tab width − `2·indicatorInsetX`) and `transform: translateX`, driven by `pagerProgress`. **RTL is the one fixed bug:** position comes from tab **widths** (`w0/w1/w2`, from each `TabButton`'s `onLayout` — never an `x`/`measureInWindow`/analytic mirror) in logical/child order: `logicalLeft = interpolated-tab-centre − chipW/2`, applied as `translateX = isRTL ? −logicalLeft : +logicalLeft`. This app RTL-swaps `left`, so `left:0` lands at the row's start and that sign (the proven-correct `marginStart = logicalLeft` equivalence) is right in both directions; `transform` is immune to the RTL swap so the sign is the only directional term. opacity 0 until measured; widths animate over `TAB.collapseDuration` on side-tab expand/collapse. **Trade-off (accepted, do not "fix" by changing the model): `width` is a per-frame layout prop, so the pager's release-settle can feel slightly rough on low-end devices — this is the cost of a full-width resizing chip and the user chose it over fixed-width / scaleX alternatives.** Do NOT reintroduce `measureInWindow`/analytic-RTL-mirror/`start:0`/flex-child-`marginStart` (each broke RTL or stuttered), and do NOT replace with per-tab opacity (not real movement) — unless the user explicitly asks to revisit the smoothness trade-off.** The chip is a fixed **single-line capsule** (height `rowHeight + 2*TAB.indicatorPadV`, `borderRadius = TAB.indicatorRadius` = half that → true capsule) over the main row only; it deliberately does **not** grow to enclose the sub-label timer. The timer instead floats as a caption *above* the chip: `subLabelOuter.bottom = TAB.rowHeight + TAB.indicatorPadV + SM`, i.e. chip-top + an `SM` gap, so the chip never clips it and the two read as a tidy pill with a caption over it.
...
- `pagerProgress` is driven by PagerView `onPageScroll(position + offset)`. Chips and the menu-tab pause indicator stay un-animated — they carry semantic state and should be equally readable whether or not their tab is selected.
...
| `starts_at` | timestamptz | when the app becomes usable inside this zone; default `now()` |
| `enabled` | boolean | default `true`; disabled zones are ignored by the gate |

Indexes: GIST on `center` (`areas_center_gix`); partial index on `enabled`.
...
- **No enabled areas at all** → `available`. Gate is OFF; identical to pre-feature behaviour. **Fully backward compatible.**
- **`location` is null** (onboarding / permission not yet granted) → `available` (never gate a user we can't place).
- Inside an enabled area with `starts_at <= now()` → `available`.
- Inside an enabled area but the earliest matching `starts_at` is in the future → `not_yet` (+ that `starts_at`).
- Inside no enabled area → `unavailable`.
...
- **Mobile (`home.tsx`):** when `relations.availability.state` is `unavailable`/`not_yet` (and the user is not in an active chat), the rotating-headline slot shows a single fixed gate message (`home.geoGate.unavailable` / `home.geoGate.notYet`), the find/play button is suppressed, and the **side tab is removed + the pager locked to it** so page2/chat is unreachable. `not_yet` lifts itself client-side when `starts_at` passes (reconfirmed on the next `start`/`focus`). An active chat is never gated (non-destructive: a user who matched before being gated keeps the conversation).

Helper SQL: `area_state(loc geography) → jsonb`, `area_available(loc geography) → boolean`, `app_availability(me_id uuid) → jsonb` (`{user, notify}` envelope). Note: `app_refresh_snapshots` does **not** recompute availability — location only changes via `start`/`location`/`focus`, and admin area changes / `not_yet` expiry propagate within ~60s via the next periodic `/app/location` or on app `focus`/launch.
