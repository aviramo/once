...
- **The drawer TRACKS THE FINGER while opening**, exactly as it tracks it while closing (user decision 2026-07-20: "ממש כמו תפריט צד בכל אפליקציה אחרת"). It is one continuous position, never a gesture that fires a canned animation. Mechanics:
  - **One `pullY`, both directions.** `menuPull` is created in `home.tsx` and handed to the sheet through OverlaySheet's `pull` prop, because the two pans live on different views: closing on the sheet, opening on the shell (a closed sheet is off-screen and catches nothing). `pullY` is distance-from-open — 0 out, `screenSpan` hidden — so the opening drag is just `screenSpan - travel`. With an external pull the HOST also owns the rest position: OverlaySheet's reset-on-open is skipped, so the hamburger path calls `menuPull.reset()` itself (`openMenuByTap`) or the tapped drawer mounts parked off-screen and is never seen.
  - **`menuDragging` mounts the sheet before it joins the stack** — a sheet must exist to be dragged in. It enters `overlays` only on commit, so an abandoned drag leaves nothing behind for the BackHandler or the gate to trip over. `animateEnter={!menuDragging}` keeps the slide-in for the tap and suppresses it for the drag, where the finger supplies the motion.
  - **Clear that flag on EVERY exit path.** First version cleared it only from the cancel animation's completion callback; an interrupted animation never fires it, so the flag stuck true, the sheet stayed mounted at `pullY` 0, and neither the X nor Back could close it (`menuOpen` was already false — there was nothing left to close). The cancel branch now unmounts immediately and lets `RisingCard`'s exit carry the sheet off, with `onFinalize` as the backstop for pans that never reach `onEnd`.
