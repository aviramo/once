...
- The **30-minute window MUST stay in sync** with the `app_add` cooldown check in `app_add` / `app_cancel_add` / `app_lock2` (each inlines `interval '30 minutes'`; the codebase does not extract this interval). If that cooldown ever changes, change this predicate in lockstep.
