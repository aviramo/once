...
Migration `20260525030000_drop_permissions_catalog` drops both tables and the `user_has_permission(uuid, text)` helper. The `setGroupPermission` server action and `web/src/app/[lang]/admin/roles/[groupId]/_components/GroupPermissions.tsx` were deleted; `getOwnerUser` / `userHasPermission` were removed from `web/src/lib/admin-auth.ts`. The admin gate stays JWT-based (`app_metadata.role='admin'`) exactly as before — it never went through these tables.
...
**Promote / demote actions** live in `web/src/app/[lang]/admin/roles/actions.ts`:
...
- `/admin/users` (list) — every query intersects with `secondary.scopeUserIds` (`scope.userIds` when manager, null when admin). Empty managed set → returns zero rows via `NO_MATCH_ID`. The role-filter dropdown's catalog is also clipped to the manager's managed groups.
- `/admin/users/[userId]` — 404 if the target isn't in `scope.userIds`. Inside the page the danger zone, group-chip editor and page-release badges receive `readOnly` and render inert for managers.
- `/admin/roles` (list) — query is restricted via `.in("id", scope.groupIds)`. `RolesManager` is rendered with `readOnly` (no checkbox, no bulk-action bar) and the "create group" form is hidden.
- `/admin/roles/[groupId]` — 404 if not in `scope.groupIds`. Members are server-sorted **managers first** (by `granted_at` asc), then non-managers by name. `GroupHeader` and `GroupDangerZone` are hidden / inert for managers. `GroupMembers` receives `canMutate` (admin only), `canPromote` (admin OR manager-of-this-group), `canDemote` (admin only) and renders the manager badge + promote/demote chips per row.
- `/admin`, `/admin/areas`, `/admin/reports` — admin-only; managers are redirected to `/admin/users`.
...
**Login flow.** `/admin/login` now branches on `getViewerScope()`: a signed-in admin redirects to `next`, a signed-in manager always redirects to `/admin/users` (their only landing screen — defensive against a deep-link `next` pointing at an admin-only surface), neither → sign out + `?error=not_admin`. Replaces the older "JWT admin or bust" check that loops a non-admin Google sign-in forever.
...
- `public.user_availability(uid uuid, loc geography) → jsonb` — **single source of truth** for a user's effective availability. **Live precedence (2026-05-25):** (1) `push_blocked(uid)` ⇒ `unavailable, reason:'push'`; (2) `loc IS NULL` ⇒ `available` (onboarding escape hatch); (3) `in_enabled_group(uid)` ⇒ `available`; (4) **no groups at all** ⇒ `available` (user request: a user with no groups is unrestricted); (5) else (in groups but every one disabled) ⇒ `unavailable, reason:'group'`. The `join_requested` sub-field was removed. `role_blocked`/`group_blocked`/`area_state` are not consulted.
...
**Group-membership gate (2026-05-25 — current).** Live precedence in `public.user_availability(uid, loc)`:

1. `push_blocked(uid)` ⇒ `{state:'unavailable', reason:'push'}`.
2. `loc IS NULL` ⇒ `available` (onboarding / pre-permission escape hatch — `push_blocked` is FALSE when `loc IS NULL` so this is safe).
3. `in_enabled_group(uid)` ⇒ `available`.
4. **No groups at all** ⇒ `available` (user decision 2026-05-25: a user with no group memberships is unrestricted — membership is organisational, not a gate by itself).
5. Else (the user IS in groups but every one of them is disabled) ⇒ `{state:'unavailable', reason:'group'}`.

`others()` mirrors the same rule: `AND (NOT only_available OR public.in_enabled_group(other.user_id) OR NOT EXISTS (SELECT 1 FROM public.user_groups ug WHERE ug.user_id = other.user_id))`. The `push_blocked` and credits-affordability clauses are preserved.

Consequences: the only way to gate a user via groups is to put them in groups and then disable every one of them (or disable the only group they hold). Removing every membership = no gate. The earlier "must be in ≥1 enabled group" rule (shipped 2026-05-19) is **superseded**.
