...
- `pagerProgress` is driven by PagerView `onPageScroll(position + offset)` and feeds `tabProgress` (= `pagerProgress` blended toward `HOME_PANE` by `profileSheetProgress`), which drives **both** the label/typography cross-fade **and** the selected chip (position + size) — one live, linear, synced motion (see the indicator bullet). The Home-tab pause/resume accessory (see its bullet) is now rendered INSIDE the morph band as part of the wordmark, so it **does** ride this exact cross-fade — its opacity is byte-identical to the name's in every state; the whole tab (not the glyph) carries the action.
...
type Page1 = {
  ...
  profile?: Profile;           // present when state ∈ {watching, waiting, chat}, and on locked when carrying a "what happened" message. While watching it is the BACK-COMPAT MIRROR of profiles[0] (the visible top) — see "page1 candidate STACK".
  profiles?: Profile[];        // watching only: the candidate STACK (up to STACK_SIZE=10). profiles[0] === profile. Absent in every non-watching state. See "page1 candidate STACK" + BACKWARD_COMPAT.md.
...
**page1 candidate STACK (Tinder/Bumble; migration `20260519140000_page1_stack` + `..._skip_topup`).** Supersedes the earlier `20260519130000_app_find_lookahead` "two wins" (the `lookahead` envelope key + the api.ts look-ahead prefetch are **removed**; the stack replaces them). `relations.page1` gains `profiles: Profile[]` — the candidate stack, present **only while `state='watching'`** (up to `STACK_SIZE`=10). The deployed app reads the single `page1.profile`, so this is staged **Expand**: while watching the server writes BOTH `profiles[]` AND `profile = profiles[0]` (the back-compat mirror = the visible top). See `BACKWARD_COMPAT.md`. All non-watching states (`waiting`/`chat`/`locked`) keep the single `profile`, no `profiles` (the stack is watching-only).

- **`_page1_pick(me_row, exclude_ids[], lim)`** — shared candidate ranking helper (the pending-inviter + "users whose TOP is me" mirror exclusions are intrinsic; the variable stack/`profile` exclusion is `exclude_ids`). Used by `app_find` + `app_skip`.
- **`app_find`** — FRESH branch (state free/locked, or watching with empty stack): picks **top-`STACK_SIZE`** via `_page1_pick`, writes `page1={state:'watching', profiles:[…], profile:profiles[0]}`, re-validates only `profiles[0]` (deeper entries are best-effort, dropped-head loop), registers the actor as a viewer on **`profiles[0]` only** (idempotent append to that user's `page2.profiles[]`), detaches from the previous top. REFILL branch (already watching, non-empty stack — new-client safety net): appends fresh picks (excluding the current stack) up to `STACK_SIZE`, top/viewer untouched. No `lookahead` key — `{user, notify}` only.
- **`app_skip(me_id, skipped_id)`** + **`POST /app/skip`** (the per-skip advance; idempotent): 24h `ignore` on the skipped, splice it out of `page1.profiles[]`, move the viewer registration (`_remove_from_profiles(skipped, me)` + append me to the new `profiles[0]`'s `page2.profiles[]`), then **top the stack back to `STACK_SIZE` in the SAME call** (user decision — the per-skip call already happens, so it appends the new tail card; the stack stays at 10 forever and **no separate "near the end" refill request is ever needed**; also self-heals shrinkage from silent kick-splices). Empty after splice + nothing to refill ⇒ `page1={state:'free'}` (same as no-candidate). No-op (returns current row) if not `watching`.
- **`app_ignore`** = `app_skip(me, current top)` (old-client skip path: advances the single `page1.profile` mirror exactly like a stack pop, so old builds keep working unchanged).
- **`_kick_page1_at` first-vs-rest rule:** a kicked user where `target` is their **top** (`page1.profile`/`profiles[0]`) → locked + message exactly as before (stack discarded, the "what happened" card; only these are RETURNED ⇒ only they get a push). `target` buried **deeper** in their stack → silently spliced out of `page1.profiles[]`, `state` untouched, no push (the user never sees it). So **first-card-becomes-irrelevant = today's behavior; non-first = silent removal** (the user's explicit requirement).
- **`app_refresh_snapshots`** also rebuilds every `page1.profiles[]` entry (Outward: B who has A in their stack; Inward: A's own stack), keeping `profile = profiles[0]` in sync.
- **Client:** `userStore.deriveCompat` exposes `relations.stack` (= `page1.profiles` while watching) and `match = stack[0] ?? page1.profile`; `applyServerUser`'s trusted-page1 merge (`'invoke:find'`, `lastRawRelations`/`writeCompat`) now also tags `app/skip` so the actor's own skip result (incl. the new stack) is trusted synchronously (no Realtime wait). `home.tsx` renders `stack[0]` as the interactive top inside the existing `PullPane` (the black "לא עכשיו" wake + `SlideOutDown` + pull transform are **reused verbatim**) and `stack[1]` (derived as the first stack entry ≠ the shown top, so it's consistently "the next top" across the transition) in a NEW static `PullPane behind` slot rendered beneath the wake — pre-mounted at rest, **no rise** (`RisingCard animateEnter={false}` always; the old `preloadingMatch`/`onPreloadReady` rise choreography is dormant). A skip = local instant (the behind card is already there) + fire-and-forget `invoke('app/skip',{skipped_id})`; `api.ts` `Image.prefetch`es the next `STACK_PREFETCH_AHEAD` stack cards (shared `matchImageUrls`, cache-key-exact with `MatchCard`). `STACK_SIZE` / `STACK_PREFETCH_AHEAD` live in `mobile/src/tokens.ts`; the SQL hardcodes 10 — keep in lockstep (credits/broadcast-constant discipline). Old builds (`app/ignore`, read `page1.profile`) ⇒ unchanged behavior.
...
A per-user credit wallet at `relations.credits` gates the spend-y actions. State shape: `{balance:int, tier:'free'|'pro', granted_on:'YYYY-MM-DD'|null, next_grant_at:timestamptz|null, held:int}`. It is a sibling of `last_add_at`/`availability` (economy state, not profile data) and is mutated atomically inside the same `SELECT … FOR UPDATE` transaction as the state transition that spends/refunds it. Credits are private (never embedded by `make_profile`, so they don't leak into other users' snapshots).

**Tiers (SQL `_credits_tier_cfg`):** `free` = 5/day, cap 5. `pro` = 10/day, cap 10. New users start `free`. The switch is now **one-way: free → Pro only** (user request — the downgrade button was removed from the settings stars/package popup; a Pro user gets no switch button, the popup is informational for them). `POST /app/set_tier` → `app_set_tier(me_id, new_tier)` still validates `new_tier ∈ {free,pro}`. **Upgrading to Pro immediately tops the wallet to the Pro cap (max = 10 stars)** — `app_set_tier` forces `tier='pro'` then reuses `_credits_reset_to_cap` (the admin-reset helper), so the resulting `credits` is `{balance:10, tier:'pro', held:0, granted_on:<grant day>, next_grant_at:<next 20:00>}`. The `tier='free'` branch is unchanged and non-destructive (flips only `credits.tier`; `balance`/`held`/`granted_on`/`next_grant_at` preserved verbatim — kept sane even though the UI no longer triggers it). "Max accumulation" == the tier's daily amount, i.e. the grant is `LEAST(cap, balance + daily)` (for `free`, effectively a daily reset to 5; for a freshly-upgraded Pro user already at cap 10 the next grant is a no-op `LEAST(10, 10+10)=10`). Additive/backward-compatible: brand-new RPC + endpoint, the deployed mobile build never calls it; response shape `{user, notify:[]}` unchanged.
...
**Costs (SQL `_credits_cost`):** `invite` 1, `approve` 1, `broadcast` 2. These three numbers (and the `5`/`10` tier amounts) are mirrored in **three** places that MUST stay in sync: the SQL `_credits_*` helpers (source of truth + enforcement), `mobile/src/lib/credits.ts` (display only), and the `defaultRelations` seed in `supabase/functions/user.ts` (`balance:5`). Change all three together. (Same discipline as the 30-minute broadcast window.)

**Broadcasting → accepting is free (user decision 2026-05-18).** While the approver is **currently broadcasting** (`relations.last_add_at` parses to a timestamp `> now() - interval '30 minutes'`, the same window as `app_add`), `app_approve`'s effective cost is **0**, not `_credits_cost('approve')` — the user already paid 2 stars to broadcast, so receiving/accepting an invitation during that window costs nothing. Implemented as a local `v_approve_cost` in `app_approve` (computed once from the FOR-UPDATE-locked `me_row`) used for both the precondition balance check and the `_credits_charge`. Because a 0 cost can never fail the `balance < cost` guard, a broadcasting user can always accept. The mobile accept CTA mirrors this: `ReplyingInviteCard.costCredits = broadcastActive ? 0 : CREDIT_COST.approve` and `affordable = broadcastActive || …` — the `CreditCost` badge stays visible showing **0** (deliberately not hidden, so the user sees it's free). This adds **two more inline copies** of the 30-minute broadcast-window predicate (in `app_approve`, and the `others()` credits-gate exemption below) to the lockstep set in the "Broadcast relevance boost" sync note — change all of them together if the window ever changes. Additive / not breaking: `app_approve`'s response shape is unchanged; an older mobile build just renders the static "1" badge while the server charges 0 (cosmetic, self-corrects on app update).

**Hold + refund policy.** `app_invite` (standard branch) deducts 1 **and records it in `credits.held`**. The held credit is **refunded** (`_credits_refund`: `balance = LEAST(cap, balance+held)`, `held=0`) when the invite ends with no self-teardown by the sender — `app_decline`, `app_expire_sweep` (both passes), and any `_kick_page1_at` of a still-`waiting` user (target matched / got invited elsewhere; `_kick_page1_at` refunds a kicked `waiting` user by construction, so approve/invite mass-kicks and logout/delete kicks are covered). It is **forfeited** (`_credits_clear_hold`: `held=0`, balance unchanged) on a self-teardown: `app_cancel` and `app_pause` (per the user's explicit rule "cancel → no refund") and `app_logout_cleanup`. On accept/match the hold is **consumed** (cleared, no refund): mutual `app_invite`, and `app_approve` (which also refunds the approver's *own* abandoned outgoing invite — matching elsewhere is not a self-cancel — then charges the approve cost). `app_approve` is at most net-0 for a previously-`waiting` approver.

**Findability gate.** `others()` drops any candidate whose `balance < _credits_cost('approve')` (= 1) under `only_available` (the clause `app_find`/`app_add` always pass), so nobody is shown a user who can't afford to accept — **except a currently-broadcasting candidate**, who bypasses the balance gate (`NULLIF(other.relations->>'last_add_at','')::timestamptz > now() - interval '30 minutes'` OR `balance >= cost`). Without this exemption a user who broadcast with their last 2 stars would sit at balance 0 and vanish from the pool *during their own paid broadcast* — self-defeating, since broadcasting's whole purpose is to be found and invited (and their accept is free anyway). Additive/no-op for the deployed app (others() is internal-only; no output column changed; backfill gave every existing user 5).

**Preconditions / failure landings.** Insufficient credits routes through the *existing* failure landings, not a new error path: `app_invite` < 1 → `page1=locked,message=invite` (`invite-fail`); `app_approve` < 1 (after the approver's own-hold refund) → `page2=locked,message=approve` (`approve-fail`); `app_add` < 2 → `{error:'no_credits'}` (HTTP 400). No new push codes.

**Relations-rebuild RPCs preserve credits.** `app_approve`, `app_pause`, `app_resume`, `app_logout_cleanup` rebuild `relations` with `jsonb_build_object`; each carries `credits` forward via `_credits_ensure`/`_credits_clear_hold`. New users are seeded `{balance:5,tier:'free',held:0}` by `user.ts`; `granted_on`/`next_grant_at` fill on the next cron tick (≤60s); `_credits_ensure` defensively seeds any credit-touching RPC that meets a credit-less row.

**Relations-rebuild RPCs also preserve the gate keys (migration `pause_resume_keep_availability` `20260519140000`).** `app_pause` / `app_resume` previously rebuilt `me`'s `relations` carrying only `credits` — silently **dropping `availability`, `push`, `join_request`**. Bug: a gated user (not in any enabled group, or push-blocked) who paused→resumed lost the gate (the mobile client defaults a missing `availability` to `available` ⇒ `geoGated=false` ⇒ the play button reappears, and the edge resume auto-find guard `availabilityState==='available'` also passes ⇒ candidates returned). Both now merge, on top of the page1/page2 skeleton, `jsonb_strip_nulls(jsonb_build_object('credits',…, 'availability', public.user_availability(me_id, location), 'push', relations->'push', 'join_request', relations->'join_request'))` — `availability` **recomputed** (the membership/push gate re-asserts immediately), `push`/`join_request` carried verbatim (`strip_nulls` ⇒ no `"key":null` when absent). Same discipline `app_admin_reset` already follows. Verified: post-pause and post-resume a no-group user keeps `{state:'unavailable',reason:'group'}`. Additive / response shape unchanged. (`app_approve`/`app_logout_cleanup` end the user in chat/logged-out states where the gate is moot, so they were left as-is; revisit if a gate-relevant rebuild is added there.)

**Admin reset tops credits to the tier cap (does NOT preserve, does NOT demote).** Both `app_admin_reset` overloads (global no-arg + role/group-scoped `p_group_ids`, the web entry point) rebuild `relations.credits = _credits_reset_to_cap(old relations)`: **`tier` (the package) is preserved** from the user's pre-reset credits (a credit-less legacy row resolves to `free` via `_credits_ensure`), **`balance` = that tier's cap** (free 5 / pro 10 — the max), `held = 0`, `granted_on`/`next_grant_at` refreshed to the current grant day (already at cap ⇒ the next 20:00 grant is a no-op `LEAST(cap,cap+daily)=cap`). This supersedes the earlier "global reset seeds a fresh `_credits_default()`" behavior, which silently demoted every `pro` user to `free`/5 — and the even-earlier regression where the roles/rename migrations dropped the `credits` key entirely (lazy fallback to free/5). User decision 2026-05-18: "an admin reset tops the star balance to the package max; do not reset the package." `app_admin_reset` is admin/service-role only (web admin → `resetUsersByRoles`); mobile never calls it, so this is additive / not breaking.

**UI.** The cost rides as an in-button badge (`CreditCost`: a rounded capsule with a coin glyph, `×`, and the number) in place of the old icon: the invite/approve accept CTA (`ReplyingInviteCard`) and the broadcast confirm popup (`ConfirmDialog.confirmIconStart`, a deliberate opt-in exception to the "buttons carry no icon" rule). The balance rides as the Menu tab's `subLabel` number (same slot as the side tab's viewer count; suppressed while the profile sheet is open). Settings has a dimmed, inert `קרדיטים` row under the Account row showing the balance and, as a small subtitle, the next-grant time (`formatNextGrant`).
...
| `relations` | jsonb | `Pages` (see Game Logic) plus a top-level `last_add_at` (ISO timestamp; 30-minute cooldown for the page2 "Show me to people" / broadcast button — see `app_add`), a top-level `availability` (geo-gate state; see "Geo-availability gate" below), a top-level `credits` (`{balance:int, tier:'free'\|'pro', granted_on:'YYYY-MM-DD'\|null, next_grant_at:timestamptz\|null, held:int}`; see "Credits economy" below), and a top-level `push` (`{perm?:'granted'\|'denied'\|'undetermined', token?:bool, dead?:bool, checked_at?:timestamptz}`; notification-presence signal, see "Notification-presence gate" below). Source of truth for page1/page2. |
