-- Handing the group over leaves you a MANAGER, not a plain member (user
-- directive 2026-07-28), reversing the one line 20260728190000 wrote into
-- app_transfer_owner. The person who built the group keeps the one job a
-- manager has (answering join requests, removing plain members) unless the new
-- owner takes it away; what he loses is the group's config and the roster's
-- appointments, which are the owner's alone.
--
-- Everything else in the RPC is unchanged: the new owner's own manager grant is
-- still cleared (an owner is never also a manager, and app_set_manager refuses
-- to touch the owner's row, so a leftover grant could never be cleared again).
--
-- Succession on account deletion (app_delete_cleanup) is untouched: the old
-- owner there is a row on its way out.
