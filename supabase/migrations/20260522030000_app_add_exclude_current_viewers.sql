-- app_add: never re-select a user who is already viewing me.
-- The previous body only excluded candidates whose page1 was currently
-- 'watching' me. The real viewer list is A.page2.profiles[]; a stale/desynced
-- entry there was not caught, so a current viewer could be pulled in again as
-- a "new" broadcast candidate. Now any user already present in
-- A.page2.profiles[] is excluded from candidate selection.
...
BEGIN
  ...
  -- users already viewing me (current page2 viewer list) must not be re-selected
  viewer_ids := ARRAY(
