-- 2026-05-13: allow broadcast while viewers already exist.
--
-- Previously app_add rejected with 'already_has_viewers' whenever
-- page2.profiles[] was non-empty. The intent was to gate broadcast as
-- an "empty pool only" booster, but in practice users on the visible
-- toggle with watchers in the list saw a server error when tapping
-- "broadcast" — the third toggle segment looked available, then failed.
--
-- New behavior:
--   * The 'already_has_viewers' precondition is removed (both the
--     pre-lock probe and the post-lock recheck).
--   * The page2 UPDATE preserves the existing profiles[] when state was
--     already 'free'; the auto-reset to {state:'free', profiles:[]}
--     still fires when state was 'locked' (lock-resting → broadcast
--     in one call). New candidates are then appended on top of the
--     existing watchers via the per-candidate UPDATE below.
--   * page2.profile presence (state='pending', a live incoming invite)
--     still rejects with 'page2_has_profile' — broadcasting on top of
--     an inbound invitation makes no sense.
--   * Candidate selection already excludes anyone currently watching me
--     (the NOT EXISTS filter), so no duplicates.
...
BEGIN
  ...
  -- Preserve existing watchers when state was already 'free'; auto-reset
  -- to {state:'free', profiles:[]} only when coming from 'locked' (the
  -- "broadcast me out of resting state" path).
  UPDATE public.users
