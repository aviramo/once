-- app_invite: stop wiping the actor's own page2.
-- Page1 buttons must not touch the actor's page2 — when A invites B,
-- A.page2 (state + profiles array) is left untouched. Same in the mutual
-- match branch for both A and B (their page1 buttons drove the match;
-- their own page2 stays as-is). Only B.page2 is written (it becomes the
-- pending invitation) — that's a write to the *other* user's page2,
-- which is what `invite` is fundamentally about.
