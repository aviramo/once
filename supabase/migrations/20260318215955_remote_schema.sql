...
alter table "public"."users" drop column "is_visible";
...
CREATE OR REPLACE FUNCTION public.others(user_id uuid, location extensions.geography)
 RETURNS TABLE(user_id uuid, "user" json, other_id uuid, distance integer, state text, relevance_gender double precision, relevance_restiction double precision, relevance_age double precision, relevance_location double precision, relevance_time double precision, relevance_subscription double precision, relevance_watchers double precision, relevance double precision)
 LANGUAGE sql
 SECURITY DEFINER
 SET search_path TO ''
AS $function$
...
select *,
relevance_gender * relevance_restiction * relevance_age * relevance_location * relevance_time * relevance_subscription * relevance_watchers relevance 
from relations
