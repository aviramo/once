-- Adds a `relevance_kids` multiplier (0..1) based on whether two users'
-- `data.family.isForKids` preferences are compatible. Folded into `others`
-- as part of the final relevance product. Server-side ranking only — not
-- displayed in the UI.
--
-- Truth table (per user request):
--   both explicitly set, same value         → 1.0
--   both explicitly set, different values   → 0.0
--   one set, the other not                  → 0.5
--   neither set                             → 1.0
...
LANGUAGE sql IMMUTABLE PARALLEL SAFE AS $$
  SELECT
    CASE
      ...
      END
      WHEN jsonb_typeof(me_data->'family'->'isForKids') = 'boolean'
        OR jsonb_typeof(other_data->'family'->'isForKids') = 'boolean'
      THEN 0.5
      ELSE 1.0
    END
$$;

-- Re-emit `others` with the new column and folded into the final product.
