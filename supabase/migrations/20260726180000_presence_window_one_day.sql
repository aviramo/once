-- 2026-07-26 — a user who has not opened the app in a full day is not shown.
--
-- The app's promise is real-time presence: the person on the card can answer an
-- invitation NOW. others() already gated on presence (not in chat, page2 open,
-- has a photo, push-reachable, can afford to approve) but never on being AROUND.
-- The only recency signal was the relevance_time WEIGHT, and it decays over a
-- YEAR: someone last seen 300 days ago still scored ~0.18 and was drawn like
-- anyone else. Every invitation sent to them expired unanswered, at the cost of
-- the inviter's credit.
--
-- So: a hard filter, not a weight, and inside the `only_available` branch — the
-- branch that already owns every "can this person be shown right now" rule. The
-- unfiltered call (only_available = false) is the admin/diagnostic view and is
-- deliberately untouched. `last_seen` is bumped by user.persist on EVERY request
-- the app makes, so a returning user is eligible again the moment they open it.
--
-- Covers every display surface, because all three draws share this pool:
-- app_find (the card), app_seed_viewer (the seeded viewer), app_add (broadcast
-- recipients — retired on the client, RPC still live).
--
-- Deliberately NO index on last_seen: it is written on every single request, so
-- an index would tax every write, and at this table size the planner would not
-- use it anyway.
--
-- Client-invisible: the pool shrinks, no response shape changes. Nothing to
-- stage in BACKWARD_COMPAT.md.

-- The window, in one place, mirroring _watch_ttl(). Tune here, not at call sites.
CREATE OR REPLACE FUNCTION public._presence_ttl()
 RETURNS interval
 LANGUAGE sql
 IMMUTABLE
AS $function$ SELECT interval '1 day'; $function$;

-- Live body as of 2026-07-26 + the one new only_available condition.
