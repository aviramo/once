-- The group is handed only to someone who already RUNS it (user directive
-- 2026-07-30): ownership passes through management, so the owner promotes
-- first and hands over second. A plain member is not a candidate — the key is
-- offered on a manager's page and nowhere else, and this guard is the same line
-- drawn server-side (`not_manager`).
--
-- Succession on account deletion (app_delete_cleanup) already reads this way
-- and is left alone: it prefers the longest-standing manager and only falls
-- back to a plain member when the group has none, because an ownerless group
-- can never be managed back out of that state. A voluntary handover has no such
-- pressure — the owner can promote someone first.
--
-- Everything else in the RPC is unchanged (20260729130000): the caller drops to
-- MANAGER, the new owner's own manager grant is cleared, both cached community
-- summaries are refreshed by hand, and the response carries `user`, the fresh
-- `members` roster and the `group_owner` notify.
...
BEGIN
  ...
  END IF;
  -- Only a manager may receive the group.
  IF NOT EXISTS(SELECT 1 FROM public.group_managers
                 WHERE group_id = p_group_id AND user_id = p_user_id) THEN
  ...
  -- ...and the outgoing owner takes that role himself (user directive
  -- 2026-07-28). His user_groups row is untouched by the handover, so
  -- _gm_ensure_member is satisfied.
