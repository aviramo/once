drop function if exists "public"."others"(user_id uuid, location extensions.geography);

alter table "public"."chat" add column "event" text;

alter table "public"."users" drop column "coords";

alter table "public"."users" drop column "images";

alter table "public"."users" drop column "is_location_active";

alter table "public"."users" drop column "lang";

alter table "public"."users" drop column "message";

alter table "public"."users" drop column "os";

alter table "public"."users" drop column "subscription";

alter table "public"."users" drop column "units";

alter table "public"."users" add column "data" jsonb not null default '{}'::jsonb;
...
CREATE OR REPLACE FUNCTION public.others(me public.users)
 RETURNS TABLE(user_id uuid, "user" json, other_id uuid, distance integer, state text, relevance_gender double precision, relevance_restiction double precision, relevance_age double precision, relevance_location double precision, relevance_time double precision, relevance_active double precision, relevance_watchers double precision, relevance double precision)
 LANGUAGE sql
 SECURITY DEFINER
 SET search_path TO ''
AS $function$
...
relations as (
  select
    ...
    case when other.is_active is null then 0 else 1 end relevance_active,
...
select *,
relevance_gender * relevance_restiction * relevance_age * relevance_location * relevance_time * relevance_active * relevance_watchers relevance 
from relations
