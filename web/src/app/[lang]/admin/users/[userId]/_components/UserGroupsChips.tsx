 ...
 * pill (disabled groups read line-through-muted, same convention as the
 * users-list card so the two screens look the same). Clicking ANY chip — or
 * the trailing "+" affordance — opens a popover-attached version of the
 * existing UserRolesEditor checklist so the operator can toggle group
 * membership in place. Clicking outside the wrapper, or Escape, closes the
 * popover.
 *
...
export function UserGroupsChips({
  ...
  const assignedRoles = roles.filter((r) => assignedSet.has(r.id));
